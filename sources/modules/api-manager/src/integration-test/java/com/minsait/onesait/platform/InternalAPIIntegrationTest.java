...
public class InternalAPIIntegrationTest {
    ...
    public void When_GetByIdIsInvoked_Then_OneInstanceIsFetched() {
        ...
        assertTrue(responseSingleResult.getBody().get(0).path(sensorTag.getOntology().getIdentification()).path(
                "id").asText().equals(instance.path(sensorTag.getOntology().getIdentification()).path("id").asText()));
    ...
    public void When_UpdateByIdIsInvoked_Then_OneInstanceIsUpdated() {
        ...
        assertTrue(responseSingleResult.getBody().path(sensorTag.getOntology().getIdentification()).path(
                JSON_PATH_TEMPERATURE).asInt() == instance.path(sensorTag.getOntology().getIdentification()).path(
                JSON_PATH_TEMPERATURE).asInt());
