...
public class ApiServiceRest {
    ...
    private static final String NOT_ALLOWED_USER_OPERATION = "com.indra.sofia2.web.api.services" +
            ".NoPermisosOperacionUsuario";

    public List<Api> findApisByUser(String userId, String token) {

        return apiRepository.findByUser(apiSecurityService.getUser(userId)).stream().filter(
                a -> apiSecurityService.authorized(a, token)).collect(Collectors.toList());
    }
    ...
    public void createApi(ApiDTO apiDTO, String token) {
        ...
        api.setUser(user);
        api.setEndpoint(resourcesService.getUrl(Module.APIMANAGER,
                                                ServiceUrl.BASE) + "server/api/v" + api.getNumversion() + "/" + api.getIdentification());
    ...
    private boolean authorizedOrSuscriptor(Api api, String tokenUsuario, String suscriptor) {
        ...
        return (apiSecurityService.isAdmin(user) || user.getUserId().equals(
                api.getUser().getUserId()) || user.getUserId().equals(suscriptor));
    }
