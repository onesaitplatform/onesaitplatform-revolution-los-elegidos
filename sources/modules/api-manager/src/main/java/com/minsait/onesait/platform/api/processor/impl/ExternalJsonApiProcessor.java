...
public class ExternalJsonApiProcessor implements ApiProcessor {
    ...
    private void proxyHttp(Map<String, Object> data) {
        ...
        try {
            ...
            data.put(Constants.REASON,
                     "Error while processing swagger for external endpoint, please review your Swagger client " +
                             "definition (schemes, basePath, etc");
    ...
    private String getUrl(Swagger swagger, String pathInfo) {
        ...
        if (swagger.getSchemes() == null || !swagger.getSchemes().stream().map(s -> s.name()).collect(
                Collectors.toList()).contains(Constants.HTTPS))
