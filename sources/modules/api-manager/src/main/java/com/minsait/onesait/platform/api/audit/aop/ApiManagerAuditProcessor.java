...
public class ApiManagerAuditProcessor {
    ...
    public ApiManagerAuditEvent getStoppedEvent(Map<String, Object> data) {
        ...
        try {
            if (ChainProcessingStatus.STOP.equals(data.get(Constants.STATUS))) {
                ...
                event = ApiManagerAuditEvent.builder().id(UUID.randomUUID().toString()).module(Module.APIMANAGER).type(
                        EventType.APIMANAGER).operationType(operation).resultOperation(
                        ResultOperationType.ERROR).remoteAddress(remoteAddress).message(reason).data(body).ontology(
                        ontologyId).query(query).timeStamp(today.getTime()).user(userId).api(
                        api != null ? api.getIdentification() : null).formatedTimeStamp(
                        CalendarUtil.builder().build().convert(today)).build();
            }
    ...
    public ApiManagerAuditEvent getEvent(Map<String, Object> data) {
        ...
        return (ApiManagerAuditEvent.builder().id(UUID.randomUUID().toString()).module(Module.APIMANAGER).type(
                EventType.APIMANAGER).operationType(operation).resultOperation(
                ResultOperationType.SUCCESS).remoteAddress(remoteAddress).message(message).data(body).ontology(
                ontologyId).query(query).timeStamp(today.getTime()).user(userId).api(
                api != null ? api.getIdentification() : null).formatedTimeStamp(
                CalendarUtil.builder().build().convert(today)).build());
