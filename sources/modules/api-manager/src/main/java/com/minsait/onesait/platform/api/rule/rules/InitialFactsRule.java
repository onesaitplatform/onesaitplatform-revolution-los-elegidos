...
public class InitialFactsRule {
    ...
    public void setFirstDerivedData(Facts facts) {
        ...
        final String queryType = Optional.ofNullable(rUtil.getValueFromRequest(Constants.QUERY_TYPE, request)).orElse(
                QueryType.NONE.name());
        ...
        final String queryDb = Optional.ofNullable(rUtil.getValueFromRequest(Constants.FILTER_PARAM, request)).orElse(
                "");
        final String targetDb = Optional.ofNullable(
                rUtil.getValueFromRequest(Constants.TARGET_DB_PARAM, request)).orElse("");
        final String formatResult = Optional.ofNullable(
                rUtil.getValueFromRequest(Constants.FORMAT_RESULT, request)).orElse("");
        final String cacheable = Optional.ofNullable(rUtil.getValueFromRequest(Constants.CACHEABLE, request)).orElse(
                "");
