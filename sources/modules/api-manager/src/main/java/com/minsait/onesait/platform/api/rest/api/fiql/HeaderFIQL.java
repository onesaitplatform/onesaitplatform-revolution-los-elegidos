...
public final class HeaderFIQL {
    ...
    private static boolean isValidCondition(String tipo) {
        return (tipo.equalsIgnoreCase(API_CONSTANTE) || tipo.equalsIgnoreCase(API_OPCIONAL) || tipo.equalsIgnoreCase(
                API_REQUERIDO));
    }

    private static boolean isValidType(String tipo) {
        return (tipo.equalsIgnoreCase(API_STRING) || tipo.equalsIgnoreCase(API_NUMBER) || tipo.equalsIgnoreCase(
                API_BOOLEAN));
    }
