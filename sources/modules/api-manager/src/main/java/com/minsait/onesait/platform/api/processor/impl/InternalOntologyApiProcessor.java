...
public class InternalOntologyApiProcessor implements ApiProcessor {
    ...
    private Map<String, Object> processQuery(Map<String, Object> data) throws JSONException {
        ...
        final OperationModel model = OperationModel.builder(ontology.getIdentification(),
                                                            ...
                                                            user.getUserId(), OperationModel.Source.APIMANAGER).body(
                body).queryType(QueryType.valueOf(queryType.toUpperCase())).objectId(OBJECT_ID).deviceTemplate(
    ...
    private void processQueryResult(OperationResultModel result, Map<String, Object> data,
            OperationType operationType) throws JSONException {
        ...
        if ("ERROR".equals(result.getResult())) {
            ...
            data.put(Constants.HTTP_RESPONSE_CODE, HttpStatus.INTERNAL_SERVER_ERROR);
            final String messageError = ApiProcessorUtils.generateErrorMessage("ERROR Output from Router Processing",
                                                                               "Stopped Execution, Error from Router",
                                                                               result.getMessage());
            data.put(Constants.REASON, messageError);
        } else {
