...
public interface TokenUserRestService {
    ...
    @ApiOperation(value = "Find token by ID", notes = "Find token by ID", httpMethod = "GET", produces =
    ...
    @POST
    @Path("{identification}/tokenUser")
    @ApiOperation(value = "POST token by ID", notes = "POST token by ID", httpMethod = "POST", produces =
            MediaType.TEXT_PLAIN, consumes = MediaType.APPLICATION_JSON, authorizations = @Authorization(value =
            "token"))
    @ApiResponses(value = {@ApiResponse(code = 204, message = "No Content"), @ApiResponse(code = 400, message = "Bad " +
            "Request"), @ApiResponse(code = 401, message = "Unauthorized"), @ApiResponse(code = 501, message =
            "Internal Server Error")})
    ...
    @PUT
    @Path("{identification}/tokenUser")
    @ApiOperation(value = "PUT token by ID", notes = "PUT token by ID", httpMethod = "PUT", produces =
            MediaType.TEXT_PLAIN, consumes = MediaType.APPLICATION_JSON, authorizations = @Authorization(value =
            "token"))
    @ApiResponses(value = {@ApiResponse(code = 204, message = "No Content"), @ApiResponse(code = 400, message = "Bad " +
            "Request"), @ApiResponse(code = 401, message = "Unauthorized"), @ApiResponse(code = 501, message =
