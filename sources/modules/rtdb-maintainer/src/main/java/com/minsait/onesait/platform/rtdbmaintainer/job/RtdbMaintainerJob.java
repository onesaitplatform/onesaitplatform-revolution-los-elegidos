...
public class RtdbMaintainerJob {
    ...
    public void execute(JobExecutionContext context) throws InterruptedException {
        ...
        final List<Ontology> ontologies = ontologyService.getCleanableOntologies().stream().filter(
                o -> o.getRtdbCleanLapse().getMilliseconds() > 0).collect(Collectors.toList());
        ...
        if (!ontologies.isEmpty()) {
            ...
            final List<CompletableFuture<String>> futureList = ontologies.stream().map(
            ...
            final CompletableFuture<Void> globalResut = CompletableFuture.allOf(
            ...
            maintenanceService.getTmpGenCollections().stream().forEach(
                    s -> maintenanceService.deleteTmpGenCollection(s));
