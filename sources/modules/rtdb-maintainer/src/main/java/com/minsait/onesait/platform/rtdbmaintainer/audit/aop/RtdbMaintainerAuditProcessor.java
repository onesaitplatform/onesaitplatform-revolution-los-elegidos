...
public class RtdbMaintainerAuditProcessor {
    ...
    public RtdbMaintainerAuditEvent getEvent(Ontology ontology, String method) {
        ...
        return RtdbMaintainerAuditEvent.builder().id(UUID.randomUUID().toString()).timeStamp(
                today.getTime()).formatedTimeStamp(CalendarUtil.builder().build().convert(today)).message(
                message).ontology(ontology.getIdentification()).user(ontology.getUser().getUserId()).module(
                Module.RTDBMAINTAINER).type(EventType.BATCH).operationType(OperationType.EXPORT.name()).resultOperation(
                ResultOperationType.SUCCESS).build();
    ...
    public RtdbMaintainerAuditEvent getEventDeleteCollection(String collection) {
        ...
        final Date today = new Date();
        return RtdbMaintainerAuditEvent.builder().id(UUID.randomUUID().toString()).timeStamp(
                today.getTime()).formatedTimeStamp(CalendarUtil.builder().build().convert(today)).message(message).user(
                AuditConst.ANONYMOUS_USER).module(Module.RTDBMAINTAINER).type(EventType.BATCH).operationType(
                OperationType.DELETE.name()).resultOperation(ResultOperationType.SUCCESS).build();
    ...
    public RtdbMaintainerAuditEvent getEventKillOp(long opId) {
        ...
        return RtdbMaintainerAuditEvent.builder().id(UUID.randomUUID().toString()).timeStamp(
                today.getTime()).formatedTimeStamp(CalendarUtil.builder().build().convert(today)).message(message).user(
                AuditConst.ANONYMOUS_USER).module(Module.RTDBMAINTAINER).type(EventType.BATCH).operationType(
                OperationType.KILL.name()).resultOperation(ResultOperationType.WARNING).build();
    ...
    public OPAuditError getErrorEvent(Ontology ontology, Exception e) {
        ...
        final String message =
                "Exception detected while exporting json from database for ontology " + ontology.getIdentification();
    ...
    public OPAuditError getErrorEventDeleteCollection(String collection, Exception e) {
        ...
        final String message = "Exception detected while attempting to delete Temporal Generated Collection by " +
                "Quasar: " + collection;
    ...
    public RtdbMaintainerAuditEvent completeEvent(Ontology ontology, String method) {
        ...
        return RtdbMaintainerAuditEvent.builder().id(UUID.randomUUID().toString()).timeStamp(
                today.getTime()).formatedTimeStamp(CalendarUtil.builder().build().convert(today)).message(
                message).ontology(ontology.getIdentification()).user(ontology.getUser().getUserId()).module(
                Module.RTDBMAINTAINER).type(EventType.BATCH).operationType(OperationType.DELETE.name()).resultOperation(
                ResultOperationType.SUCCESS).build();
    }
}
