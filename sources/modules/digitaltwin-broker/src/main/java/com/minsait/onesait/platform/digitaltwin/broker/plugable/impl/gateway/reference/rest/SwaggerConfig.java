...
public class SwaggerConfig {
    ...
    public Docket api() {
        return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.basePackage(
                "com.minsait.onesait.platform.digitaltwin.broker.plugable.impl.gateway.reference.rest")).paths(
                PathSelectors.any()).build().host(host).apiInfo(apiInfo());
    }

    private ApiInfo apiInfo() {
        final ApiInfo apiInfo = new ApiInfo("onesait Platform IoT Digital Twin Rest Gateway",
