    ...
    public void manageRule(HazelcastRuleObject rule) {
        ...
        if (droolsRule == null && StringUtils.isEmpty(rule.getDRL()))
            ...
            if (results.hasMessages(Level.ERROR)) {
                HazelcastMessageNotification.builder().rule(rule.getIdentification()).message(
                        results.toString()).build().toJson().ifPresent(m -> topicAsyncComm.publish(m));

            } else {
                HazelcastMessageNotification.builder().rule(rule.getIdentification()).message(
                        HazelcastMessageNotification.OK).build().toJson().ifPresent(m -> topicAsyncComm.publish(m));
            }
        }
    }
