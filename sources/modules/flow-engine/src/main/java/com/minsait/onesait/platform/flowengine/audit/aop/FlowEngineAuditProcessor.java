...
public class FlowEngineAuditProcessor {
    ...
    public FlowEngineAuditEvent getEvent(String domain, String user, OperationType operation, String message,
        ...
        return FlowEngineAuditEvent.builder().id(UUID.randomUUID().toString()).module(Module.FLOWENGINE).domain(
                domain).type(EventType.FLOWENGINE).operationType(operation.name()).timeStamp(today.getTime()).user(
                user).message(message).resultOperation(result).formatedTimeStamp(
    ...
    public OPAuditError getErrorEvent(String methodName, FlowEngineDomain domain, Exception ex) {
        ...
        String messageOperation =
                "Exception Detected while executing " + methodName + " for domain : " + domain.getDomain();
