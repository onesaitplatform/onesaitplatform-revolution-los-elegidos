        ...
        while (fields.hasNext()) {
            ...
            switch (function) {
                    ...
                    if (map.at(path).isArray())
                        ((ArrayNode) map.at(path)).insert(Integer.parseInt(finalField),
                                                          randomizeDouble(json.path(field).get("from").asDouble(),
                                                                          json.path(field).get("to").asDouble(),
                                                                          json.path(field).get("precision").asInt()));
                    else
                ...
                case RANDOM_INTEGER:
                    if (map.at(path).isArray())
                        ((ArrayNode) map.at(path)).insert(Integer.parseInt(finalField),
                                                          randomizeInt(json.path(field).get("from").asInt(),
                                                                       json.path(field).get("to").asInt()));
                    ((ObjectNode) map.at(path)).put(finalField, randomizeInt(json.path(field).get("from").asInt(),
                                                                             json.path(field).get("to").asInt()));
                    break;
                case COSINE_NUMBER:
                    ...
                    if (map.at(path).isArray())
                        ((ArrayNode) map.at(path)).insert(Integer.parseInt(finalField),
                                                          Math.cos(angleCos) * multiplierCos);
                    else
                ...
                case SINE_NUMBER:
                    ...
                    if (map.at(path).isArray())
                        ((ArrayNode) map.at(path)).insert(Integer.parseInt(finalField),
                                                          Math.sin(angleSin) * multiplierSin);
                    else
