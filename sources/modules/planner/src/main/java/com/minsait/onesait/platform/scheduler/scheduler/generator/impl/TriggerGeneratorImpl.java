...
public class TriggerGeneratorImpl implements TriggerGenerator {
    ...
    @Override
    public Trigger createCronTrigger(String cronExpression, JobDetail jobDetail, TriggerKey triggerKey, Date startAt,
            Date endAt) {

        CronScheduleBuilder schedBuilder = CronScheduleBuilder.cronSchedule(cronExpression).
                withMisfireHandlingInstructionDoNothing();

        TriggerBuilder<CronTrigger> builder = TriggerBuilder.newTrigger().forJob(jobDetail).withSchedule(
                schedBuilder).withIdentity(triggerKey);
