...
public class SolverSQLServerImpl implements SolverInterface {
    ...
    private boolean isSimpleDatasource(String queryOri) {
        ...
        return query.indexOf("inner join") == -1 && query.indexOf("select", 1) == -1 && query.indexOf(
                "outer join") == -1 && query.indexOf("full join") == -1;
    }
    ...
    private String[] getRealProject(String query) {
        ...
        if (query.toLowerCase().indexOf("select top", 0) != -1) {
            lsrealproject = query.substring(query.toLowerCase().indexOf(')') + 1,
                                            query.toLowerCase().indexOf(FROM)).split(",");
        } else {
            lsrealproject = query.substring(query.toLowerCase().indexOf("select ") + 7,
                                            query.toLowerCase().indexOf(FROM)).split(",");
        }
    ...
    private String buildWhere(List<FilterStt> filters, String prefix, boolean includeWhere, String[] realproject) {
        if (filters == null || filters.isEmpty()) {
            ...
            for (FilterStt f : filters) {
                ...
                sb.append(f.getOp());
                sb.append(" ");
                sb.append(isFilterOverString(f) ? (f.getExp().replaceFirst(STRING_DEBOUND_CHAR,
                                                                           STRING_LOCAL_BOUNDS_CHAR)).replaceFirst(".$",
                                                                                                                   STRING_LOCAL_BOUNDS_CHAR)
                                  : f.getExp());
