...
public class SolverOracleImpl implements SolverInterface {
    ...
    private String[] getRealProject(String query) {
        String[] lsrealproject = query.substring(query.toLowerCase().indexOf("select ") + 7,
    ...
    private String buildWhere(List<FilterStt> filters, String prefix, boolean includeWhere, String[] realproject) {
        if (filters == null || filters.size() == 0) {
            ...
            for (FilterStt f : filters) {
                ...
                sb.append(f.getOp());
                sb.append(" ");
                sb.append(isFilterOverString(f) ? (f.getExp().replaceFirst(stringDEBoundsChar,
                                                                           stringLocalBoundsChar)).replaceFirst(".$",
                                                                                                                stringLocalBoundsChar)
                                  : f.getExp());
