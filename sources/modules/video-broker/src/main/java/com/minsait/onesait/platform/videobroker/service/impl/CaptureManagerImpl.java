...
public class CaptureManagerImpl implements CaptureManager {
    ...
    public void startThread(VideoCapture videoCapture) {
        if (!executingThreads.containsKey(videoCapture.getId())) {
            ...
            final VideoProcessorParameters parameters = VideoProcessorParameters.builder().ontology(
                    videoCapture.getOntology().getIdentification()).url(videoCapture.getUrl()).frameProcessor(
                    getProcessorType(videoCapture.getProcessor())).timerStep(
                    videoCapture.getSamplingInterval()).captureName(videoCapture.getIdentification()).build();
    ...
    public void synchronize() {
        ...
        executingThreads.entrySet().removeIf(
                e -> e.getValue() == null || ((VideoProcesor) e.getValue()).isInterrupted());
    ...
    private boolean isOntologyAuthorized(Ontology ontology) {
        ...
        return cp.getClientPlatformOntologies().stream().map(ClientPlatformOntology::getOntology).collect(
                Collectors.toSet()).contains(ontology);
    }
