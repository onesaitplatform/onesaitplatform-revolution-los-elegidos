...
function getOntologyFields() {
    ...
    payload[headerJson.csrfParameterName] = headerJson.csrfToken;
    $("#ontologyFields").load('/controlpanel/devicesimulation/ontologyfields', payload, function (data) {
        editFieldParameters()
    });
    $("#tabs").show();
    $("#ontologyFields").show();
...
function submitForm(formId) {
    if ($('#interval').val() > 0 && $('#identification-form').val() != "" && $('#identification-form').val().length > 9) {
        ...
        if ($("#from-json").is(':checked')) {
            ...
            jQuery.post('/controlpanel/devicesimulation/checkjson', {
                'ontology': $("#ontologies").val(),
...
var deleteSimulation = function (id) {
    ...
    // no Id no fun!
    if (!id) {
        $.alert({title: 'ERROR!', type: 'red', theme: 'dark', content: 'NO SIMULATION SELECTED!'});
        return false;
    }
                ...
                action: function () {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result == 'ok') {
                                navigateUrl('/controlpanel/devicesimulation/list');
                            }
                        }
                    });
                }
