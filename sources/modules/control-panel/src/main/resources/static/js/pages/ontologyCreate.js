...
var OntologyCreateController = function () {
    ...
    var createJsonProperties = function (jsonData) {
        ...
        // Required
        if (jsonData.hasOwnProperty('datos')) {
            required = jsonData.datos.required;
        } else {
        ...
        // Properties
        if (jsonData.hasOwnProperty('datos')) {
            properties = jsonData.datos.properties;
        } else {
                ...
                $.each(object, function (propKey, propValue) {
                    ...
                    if (propKey == 'type') {
                        ...
                        if (object.hasOwnProperty('properties')) {
                            if (object.properties.hasOwnProperty('media')) {
                                isFile = true;
                            }
                        }
                        if (object.hasOwnProperty('properties')) {
                        ...
                        if (object.hasOwnProperty('properties')) {
                            if (object.properties.hasOwnProperty('$date')) {
                                isTimestampMongo = true;
                            }
                        }
                        if (isFile) {
                            objectType = 'file';
                        } else if (isGeometryPoint) {
                            objectType = 'geometry-point';
                        } else if (isGeometryLineString) {
                            objectType = 'geometry-linestring';
                        } else if (isGeometryPolygon) {
                            objectType = 'geometry-polygon';
                        } else if (isGeometryMultiPoint) {
                            objectType = 'geometry-multipoint';
                        } else if (isGeometryMultiLineString) {
                            objectType = 'geometry-multilinestring';
                        } else if (isGeometryMultiPolygon) {
                            objectType = 'geometry-multipolygon';
                        } else if (isTimestampMongo) {
                            objectType = 'timestamp-mongo';
                        } else if (isTimestamp) {
            ...
            if (schemaObj.hasOwnProperty("_references")) {
                ...
                references.forEach(function (r) {
                    ...
                    referencesArr.push({
                        ...
                        "dstOntology": dstOnt,
                        "dstAtt": dstAtt,
                        "srcAttPath": srcAttPath,
                        "dstAttPath": dstAttPath,
                        "validate": validate
                    });
    ...
    var mountLDModal = function () {
        ...
        var properties = $("input[name='property\\[\\]']").map(function () {
            if ($(this).val() !== '') {
                return $(this).val();
            }
        }).get();
        var propTypes = $("select[name='type\\[\\]']").map(function () {
            return $(this).val();
        }).get();
        var schemaObj = {};
        // get current schema
    ...
    var updateSchemaProperties = function () {
        ...
        var updateProperties = $("input[name='property\\[\\]']").map(function () {
            if ($(this).val() !== '') {
                return $(this).val();
            }
        }).get();
        var updateTypes = $("select[name='type\\[\\]']").map(function () {
            return $(this).val();
        }).get();
        var updateRequired = $("select[name='required\\[\\]']").map(function () {
            return $(this).val();
        }).get();
        var updateDescription = $("input[name='descriptions\\[\\]']").map(function () {
            return $(this).val();
        }).get();
        var updateEncrypted = $("select[name='encrypted\\[\\]']").map(function () {
            return $(this).val();
        }).get();
        ...
        if (updateProperties.length !== checkUnique.length) {
            $.alert({title: 'ERROR!', theme: 'light', content: ontologyCreateReg.validations.duplicates});
            return false;
        }
        ...
        if (typeof schema == 'string') {
            ...
            $.alert({title: 'ERROR!', theme: 'light', content: ontologyCreateReg.validations.tplschema});
            return false;
        }
        ...
        if (schemaObj.hasOwnProperty('datos')) {
            schemaObj.datos.properties = {};
            schemaObj.datos.required = [];
        } else {
...
// ontologyCreateReg.validations.noschema}); return false; }
        ...
        if (schemaObj.hasOwnProperty('datos')) {
            properties = schemaObj.datos.properties;
            requires = schemaObj.datos.required;
        } else {
            properties = schemaObj.properties;
            requires = schemaObj.required
        }
...
// $('#jsonschema').val(schema);
    ...
    var dataModeltemplateCounters = function () {
        ...
        datamodels.each(function (ind, elem) {
            ...
            if (templates == 0) {
                $(elem).find('div.list-toggle').removeClass('bg-blue-hoki font-grey-carrara').addClass('bg-grey-steel font-grey-cascade hide');
            }
        });
    }
    ...
    var deleteOntologyConfirmation = function (ontologyId) {
        ...
        // no Id no fun!
        if (!ontologyId) {
            $.alert({title: 'ERROR!', theme: 'light', content: ontologyCreateReg.validations.validform});
            return false;
        }
    ...
    var validateJsonSchema = function () {
        ...
        if (IsJsonString(editor.getText())) {
            ...
            if ((ontologia.properties == undefined && ontologia.required == undefined)) {
                ...
                $.alert({
                    title: 'ERROR JSON SCHEMA!',
                    theme: 'light',
                    content: ontologyCreateReg.validations.schemanoprop
                });
                isValid = false;
                return isValid;
                ...
                if ((nodo == undefined)) {
                    ...
                    $.alert({
                        title: 'JSON SCHEMA!',
                        theme: 'light',
                        content: ontologyCreateReg.validations.noproperties
                    });
                    isValid = false;
                    return isValid;
                }
    ...
    var generateProperty = function (propertyName, property, instance) {
        ...
        if (propertyName == "$oid") {
            ...
            if (property.type == "string") {
                instance = instance + "\"$oid\":\"53b281b1c91cbd35025e3d91\"";
            }
            instance = instance + ",";
        } else {
            ...
            if (tipo.includes("geometry")) {
                instance = instance + generateBasicType(tipo, "", "");
                // adding object type
            } else if (tipo.toLowerCase() == "object") {
                console.log('INSTANCE (obj): ' + instance);
                instance = instance + generateObject(property, "", propertyName);
                // adding array type
            } else if (tipo.toLowerCase() == "array") {
                console.log('INSTANCE (arr): ' + instance);
                instance = instance + generateArray(property, "", propertyName);
                // timestamp
    ...
    var generateBasicType = function (propType, obj, parent, thevalue) {
        ...
        // string
        if (propType == "string") {
            if (parent == "media" && obj == "storageArea") {
                return "\"SERIALIZED\"";
            } else if (parent == "media" && obj == "binaryEncoding") {
                return "\"Base64\"";
            } else if (parent == "media" && obj == "name") {
                return "\"fichero.pdf\"";
            } else if (parent == "media" && obj == "mime") {
        ...
        // integer,boolean, object, number and geometry
            ...
            return "{}";
        } else if (propType == "geometry-point" || obj == "geometry-point") {
            return "{\"type\":\"Point\", \"coordinates\":[9,19.3]}";
        } else if (propType == "geometry-linestring" || obj == "geometry-linestring") {
            return "{\"type\":\"LineString\", \"coordinates\":[[9,19.3],[19,9.3]]}";
        } else if (propType == "geometry-polygon" || obj == "geometry-polygon") {
            return "{\"type\":\"Polygon\", \"coordinates\":[ [[9,-19.3],[-19,-9.3],[-9,19.3],[19,-9.3]] ,[[9,19.3],[19,9.3],[-9,19.3],[-19,9.3]] ]}";
        } else if (propType == "geometry-multipoint" || obj == "geometry-multipoint") {
            return "{\"type\":\"MultiPoint\", \"coordinates\":[[9,19.3],[19,19.3]]}";
        } else if (propType == "geometry-multilinestring" || obj == "geometry-multilinestring") {
            return "{\"type\":\"MultiLineString\", \"coordinates\":[ [[9,19.3],[19,9.3]], [[9,19.3],[19,9.3]] ]}";
        } else if (propType == "geometry-multipolygon" || obj == "geometry-multipolygon") {
            return "{\"type\":\"MultiPolygon\", \"coordinates\":[[[[9,2.5],[9,2],[-9,3],[9,-3],[-9,2]]],[[[9,0],[9,-0.5],[9,-1.5],[9,1],[9,0.4]],[[9,0.2],[-9,0.2],[9,0.8],[-9,0.8],[9,3.2]]]]}";
        } else if (propType == "number" || propType == "numeric") {
            return "28.6"
        ...
        if (ontology.properties) {
            for (var obj in ontology.properties) {
                ...
                // date obj
                    ...
                    if (obj == "$date") {
                        instance = instance + "\"$date\": \"2014-01-30T17:14:00Z\"";
                    } else {
                        instance = instance + "\"" + obj + "\":" + "{\"$date\": \"2014-01-30T17:14:00Z\"}";
                    }
                }
    ...
    var authorization = function (action, ontology, user, accesstype, authorization, btn) {
        ...
        if (action === 'update') {
            ...
            var csrf_header = $("meta[name='_csrf_header']").attr("content");
            $.ajax({
                url: updateURL, type: "POST", async: true,
                headers: {
                    [csrf_header]: csrf_value
                },
        ...
        if (action === 'delete') {
            ...
            var csrf_header = $("meta[name='_csrf_header']").attr("content");
            $.ajax({
                url: deleteURL, type: "POST", async: true,
                headers: {
                    [csrf_header]: csrf_value
                },
                ...
                success: function (response, status) {
                    ...
                    // remove object
                    var removeIndex = authorizationsIds.map(function (item) {
                        return item[user];
    ...
    var foundIndex = function (what, item, arr) {
        ...
        arr.forEach(function (element, index, array) {
            if (what === element[item]) {
                found = index;
                console.log("a[" + index + "] = " + element[item] + ' Founded in position: ' + found);
            }
    ...
    return {
        ...
        checkProperty: function (obj) {
            ...
            var allProperties = $("input[name='property\\[\\]']").map(function () {
                return $(this).val();
            }).get();
            ...
            if (allProperties.length !== areUnique.length) {
                $.alert({
                    ...
                    theme: 'light',
                    type: 'red',
                    content: ontologyCreateReg.validations.duplicates
                });
                $(obj).val('');
                return false;
            } else {
        ...
        schemaToTable: function (objschema, tableId) {
            ...
            if (typeof schema == 'string') {
                ...
                $.alert({
                    title: 'ERROR!',
                    theme: 'light',
                    content: ontologyCreateReg.validations.noschemaontologyCreateReg.validations.noschema
                });
                return false;
            }
            ...
            if (($('#identification').val() == '') || ($('#description').val() == '')) {
                $.alert({
                    title: ontologyCreateReg.datamodel,
                    theme: 'light',
                    content: ontologyCreateReg.dataModelSelection
                });
                return false;
            } else {
            ...
            if ($('#datamodel_properties').attr('data-loaded') == 'true') {
                ...
                $.confirm({
                    title: ontologyCreateReg.confirmBtn,
                    theme: 'light',
                    content: ontologyCreateReg.validations.datamodelchange,
                    buttons: {
        ...
        insertAuthorization: function () {
            ...
            if (ontologyCreateReg.actionMode !== null) {
                ...
                if (($('#users').val() !== '') && ($("#users option:selected").attr('disabled') !== 'disabled') && ($('#accesstypes').val() !== '')) {
                    ...
                    $.alert({title: 'ERROR!', theme: 'light', content: ontologyCreateReg.validations.authuser});
                }
            }
        },
