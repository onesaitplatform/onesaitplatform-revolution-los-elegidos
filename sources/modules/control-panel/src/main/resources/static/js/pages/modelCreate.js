...
//	var mountableModel = $('#parameters').find('tr.mountable-model')[0].outerHTML; // save html-model for when select new datamodel, is remove current and create a new one.
    ...
    var updateSchemaProperties = function () {
        ...
        if (parameters.length !== checkUnique.length) {
    ...
    var deleteModelConfirmation = function (modelId) {
        ...
        // no Id no fun!
        if (!modelId) {
            $.alert({title: 'ERROR!', type: 'red', theme: 'light', content: modelCreateJson.validations.validform});
            return false;
        }
    ...
    var checkCreate = function () {
        ...
        if (dateDeleted != "") {
            if (dateCreated > dateDeleted) {
                $.confirm({
                    icon: 'fa fa-warning', title: 'CONFIRM:', theme: 'dark',
            ...
            submitHandler: function (form) {
                ...
                if (!$('#dashboard-radio').is(':checked') && !$('#paragraph-radio').is(':checked')) {
                    $.alert({
                        ...
                        theme: 'light',
                        type: 'red',
                        content: modelCreateJson.validations.outputSource
                    });
                    return;
                }
                ...
                if ($('#categories_select').val() == '' || $('#subcategories').val() == '') {
                    $.alert({
                        ...
                        theme: 'light',
                        type: 'red',
                        content: modelCreateJson.validations.category
                    });
                    return;
                }
        ...
        checkParameter: function (element) {
            ...
            if (parameters.length !== areUnique.length) {
                $.alert({
                    ...
                    theme: 'light',
                    type: 'red',
                    content: modelCreateJson.validations.duplicates
                });
...
//				SPINNER OFFSET INIT
                ...
                spinnerEachFrom.bind("keydown", function (event) {
...
//				SPINNER OFFSET INIT
                ...
                spinnerEachTo.bind("keydown", function (event) {
                    event.preventDefault();
                });
                        ...
                        action: function () {
                            if ($("#type").val() == '' || $("#type").val() == "select" || $("#parameter_name").val() == '' || $("#parameter_name").val() == undefined) {
                                $.alert({
                                    title: 'ALERT!',
                                    theme: 'light',
                                    content: modelCreateJson.validations.parameters
                                });
                                return;
                            }
