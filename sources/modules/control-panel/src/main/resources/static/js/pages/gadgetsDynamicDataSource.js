...
function getQueryExecutedFragment() {
    ...
    $("#Canvasrespuesta").load('/controlpanel/querytool/query', {
        'query': $("#querySql").val(),
        'queryType': $("#typeQuery").val(),
        'ontologyIdentification': ontologyIdentification
        ...
        if (IS_JSON) {
            ...
            editor.setMode('text');
            editor.setText(queryErrorResponse);
        }
    ...
    if ($('#result-panel').hasClass('hide')) {
        $('#result-panel').toggleClass('hide');
    }
}
...
function getRtdbFromOntology() {
    ...
    $.ajax({
        url: "/controlpanel/querytool/rtdb/" + datasour,
        headers: {
            [csrf_header]: csrf_value
        },
        type: 'GET',
...
function showData(key, str) {
    ...
    var str = valueObjects.find(x = > x.key === key
).
    json;
    if (str === 'null') {
        return false;
...
function changeQuery() {
    ...
    queryPanels.each(function () {
        $(this).children().not('.panel-sofia2').remove();
    });
    ...
    if (targetdb == "rtdb") {
        ...
        if ((option == "sql") && (queryKp != null) && (bdtrType != "ORACLE")) {
            $('#querySql').innerHTML = queryKp;
            $('#querySql').val(queryKp);
        } else if ((option == "native") && (queryKp != null)) {
            $('#querySql').innerHTML = "";
            $('#querySql').val("");
        } else if ((option == "native") && (bdtrType == "rtdb") && (queryKp == "null")) {
            $('#querySql').innerHTML = "db." + base_ontology + ".find().limit(3);";
            $('#querySql').val("db." + base_ontology + ".find().limit(3);");
        } else if ((bdtrType == "ORACLE") && (queryKp == null)) {
            $('#querySql').innerHTML = "select * from " + base_ontology + " where ROWNUM <= 3;";
            $('#querySql').val("select * from " + base_ontology + " where ROWNUM <= 3;");
        } else if ((option == "sql") && (queryKp == null) && (bdtrType == "rtdb")) {
            $('#querySql').innerHTML = "select * from " + base_ontology;
            $('#querySql').val("select * from " + base_ontology);
        }
...
function addSelect() {
    mostrarDialogoSelect();
}

function addCondition() {
    mostrarDialogoCondicion();
}

function addOrderBy() {
    mostrarDialogoOrderBy();
}
...
function createQuery() {
    ...
    if (ontology === '') {
        ...
        if (option == "sql") {
            console.log('createQuery()->createQuerySql');
            $('#querySql').val(createQuerySql());
        }
...
function createQuerySql() {
    ...
    if (conditions.length == 1) {
        ...
        conditions.each(function (index) {
            ...
            if (index == 0) {
                condition = "c." + conditionAux;
            } else {
                condition = condition + " AND " + "c." + conditionAux;
            }
        });
    }
    ...
    if (orderby.length == 1) {
        hasOrderBy = true;
        queryOrderBy = orderby.find('input').val();
    }

    // fields (SELECT)		
    if (fields.length == 1) {
        queryFields = fields[0].getAttribute("name");
        if ((queryFields == "Todos los campos") || (queryFields == "All fields")) {
            allFields = true;
        }
    ...
    */
    ...
    // QUERY ASSEMBLY
    if (hasCondition && hasGroupBy && hasOrderBy) {
        query = query + queryFields + " from " + ontology + aliasTable + " c where " + condition + " group by " + queryGroupBy + " order by " + queryOrderBy;
    } else if (!hasCondition && !hasGroupBy && !hasOrderBy) {
        query = query + queryFields + " from " + ontology + aliasTable + " c";
    } else if (hasCondition && !hasGroupBy && !hasOrderBy) {
        query = query + queryFields + " from " + ontology + aliasTable + " c where " + condition;
    } else if (!hasCondition && hasGroupBy && !hasOrderBy) {
        query = query + queryFields + " from " + ontology + aliasTable + " c group by " + queryGroupBy;
    } else if (hasCondition && !hasGroupBy && hasOrderBy) {
        query = query + queryFields + " from " + ontology + aliasTable + " c where " + condition + " order by " + queryOrderBy;
    } else if (!hasCondition && hasGroupBy && hasOrderBy) {
        query = query + queryFields + " from " + ontology + aliasTable + " c group by " + queryGroupBy + " order by " + queryOrderBy;
    } else if (hasCondition && hasGroupBy && !hasOrderBy) {
        query = query + queryFields + " from " + ontology + aliasTable + " c where " + condition + " group by " + queryGroupBy;
    } else if (!hasCondition && !hasGroupBy && hasOrderBy) {
        query = query + queryFields + " from " + ontology + aliasTable + " c order by " + queryOrderBy;
    }
