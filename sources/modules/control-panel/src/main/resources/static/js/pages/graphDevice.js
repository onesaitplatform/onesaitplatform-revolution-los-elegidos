var GraphDeviceController = function () {
                ...
                if ($.inArray(Node.target, avoidDuplicates) === -1) {
                    ...
                    if (Node.image === null) {
                        ...
                        dataJson = {
                            ...
                            'title': Node.title,
                            'linkCreate': Node.linkSource,
                            'status': Node.status,
                            'connected': Node.connected,
                            'sessionKey': Node.sessionKey,
                            'updateAt': new Date(Node.updateAt).toLocaleString()
                        };
                    } else {
                        dataJson = {
                            'id': Node.target,
    ...
    var loadNetwork = function () {
        ...
        $.ajax({
            url: "/controlpanel/devices/management/getgraph/", type: 'GET', dataType: 'json', contentType: 'text/html',
            headers: {
    ...
    var handleGraphHeight = function () {
        ...
        $('#btn-remH').on('click', function () {
            ...
            if (parseInt($('#networkVis').css('height')) <= 475) {
