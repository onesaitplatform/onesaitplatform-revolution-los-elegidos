var DigitalTwinCreateController = function () {
                ...
                success: function (data) {
                    if (data > 0) {
                        $.alert({
                            ...
                            theme: 'light',
                            type: 'red',
                            content: digitalTwinCreateJson.validations.update
                        });
                    } else {
    ...
    var updateSchemaProperties = function () {
        ...
        var updateTypes = $("select[name='typeProp\\[\\]']").map(function () {
            return $(this).val();
        }).get();
        var updateUnits = $("input[name='units\\[\\]']").map(function () {
            if ($(this).val() !== '') {
                return $(this).val();
            }
        }).get();
        var updateDirection = $("select[name='direction\\[\\]']").map(function () {
            return $(this).val();
        }).get();
        var updateDescription = $("input[name='descriptionsProps\\[\\]']").map(function () {
            if ($(this).val() !== '') {
                return $(this).val();
            }
        }).get();
        ...
        if (updateProperties.length !== checkUnique.length) {
            $.alert({title: 'ERROR!', theme: 'light', type: 'red', content: digitaltwintype.validations.duplicates});
            return false;
        }
    ...
    var updateSchemaActions = function () {
        ...
        var updateActions = $("input[name='action\\[\\]']").map(function () {
            if ($(this).val() !== '') {
                return $(this).val();
            }
        }).get();
        var updateDescription = $("input[name='descriptionsActions\\[\\]']").map(function () {
            if ($(this).val() !== '') {
                return $(this).val();
            }
        }).get();
        ...
        if (updateActions.length !== checkUnique.length) {
            $.alert({title: 'ERROR!', theme: 'light', type: 'red', content: digitaltwintype.validations.duplicates});
            return false;
        }
    ...
    var updateSchemaEvents = function () {
        ...
        var updateEvent = $("input[name='event\\[\\]']").map(function () {
            if ($(this).val() !== '') {
                return $(this).val();
            }
        }).get();
        var updateDescription = $("input[name='descriptionsEvents\\[\\]']").map(function () {
            if ($(this).val() !== '') {
                return $(this).val();
            }
        }).get();
        var updateTypes = $("select[name='typeEvent\\[\\]']").map(function () {
            return $(this).val();
        }).get();
        var updateStatus = $("input[name='status\\[\\]']").map(function () {
            if ($(this).val() !== '') {
        ...
        if (updateEvent.length !== checkUnique.length) {
            $.alert({title: 'ERROR!', theme: 'light', type: 'red', content: digitaltwintype.validations.duplicates});
            return false;
        }
    ...
    var deleteDigitalTwinTypeConfirmation = function (digitalTwinTypeId) {
        ...
        // no Id no fun!
        if (!digitalTwinTypeId) {
            $.alert({
                title: 'ERROR!',
                type: 'red',
                theme: 'light',
                content: digitalTwinCreateJson.validations.validform
            });
            return false;
        }
    ...
    return {
                ...
                if (eventsJson.length > 0) {
                    ...
                    $.each(eventsJson, function (key, object) {
                        ...
                        EventUpdate = {
                            ...
                            "status": status,
                            "typeEvent": type,
                            "descriptionsEvents": description
                        };
        ...
        checkProperty: function (obj) {
            ...
            if (allProperties.length !== areUnique.length) {
                $.alert({
                    ...
                    theme: 'light',
                    type: 'red',
                    content: digitalTwinCreateJson.validations.duplicates
                });
                $(obj).val('');
                return false;
            } else {
        ...
        checkAction: function (obj) {
            ...
            var allActions = $("input[name='action\\[\\]']").map(function () {
                return $(this).val();
            }).get();
        ...
        checkEvent: function (obj) {
            ...
            var allEvents = $("input[name='event\\[\\]']").map(function () {
                return $(this).val();
            }).get();
