...
var AppCreateController = function () {
    ...
    var initTemplateElements = function () {
        ...
        // EDIT MODE ACTION
            ...
            if (associationsJson.length > 0) {
                ...
                var authid_update, fatherAppId_update, fatherRoleName_update, childAppId_update, childRoleName_update,
                    associationUpdate, associationIdUpdate = '';
                $.each(associationsJson, function (key, object) {
                    ...
                    associationUpdate = {
                        "fatherApps": fatherAppId_update,
                        "fatherRoles": fatherRoleName_update,
                        "childApps": childAppId_update,
                        "childRoles": childRoleName_update,
                        "id": authid_update
                    };
    ...
    var removeRole = function (row) {
        ...
        var removeIndexFatherRole = associationsArr.map(function (item) {
            return item['fatherRoles'];
        }).indexOf(row.parentElement.parentElement.dataset.rolename);
        while (removeIndexFatherRole != -1) {
            ...
            removeIndexFatherRole = associationsArr.map(function (item) {
                return item['fatherRoles'];
            }).indexOf(row.parentElement.parentElement.dataset.rolename);
        }

        // Miramos si el rol es hijo de otro rol para eliminar la asociacion:
        var removeIndexChildRole = associationsArr.map(function (item) {
            return item['childRoles'];
        }).indexOf(row.parentElement.parentElement.dataset.rolename);
        while (removeIndexChildRole != -1) {
            associationsIds.splice(removeIndexChildRole, 1);
            associationsArr.splice(removeIndexChildRole, 1);
            removeIndexChildRole = associationsArr.map(function (item) {
                return item['childRoles'];
            }).indexOf(row.parentElement.parentElement.dataset.rolename);
        }

        var removeIndex = authorizationsArr.map(function (item) {
            return item['rolesName'];
        }).indexOf(row.parentElement.parentElement.dataset.rolename);
        while (removeIndex != -1) {
            authorizationsIds.splice(removeIndex, 1);
            authorizationsArr.splice(removeIndex, 1);
            removeIndex = authorizationsArr.map(function (item) {
                return item['rolesName'];
            }).indexOf(row.parentElement.parentElement.dataset.rolename);
        }
    ...
    var association = function (action, fatherAppId, fatherRoleId, childAppId, childRoleId, association, btn) {
        ...
        if (action === 'insert') {
            ...
            $.ajax({
                ...
                success: function (response, status) {
                    ...
                    var propAuth = {
                        "fatherApps": response.fatherAppId,
                        "fatherRoles": response.fatherRoleName,
                        "childApps": response.childAppId,
                        "childRoles": response.childRoleName,
                        "id": response.id
                    };
        ...
        if (action === 'delete') {
            ...
            $.ajax({
                ...
                data: {
                    "fatherRoleName": fatherRoleId,
                    "childRoleName": childRoleId,
                ...
                success: function (response, status) {
                    ...
                    // remove object
                    var removeIndex = associationsIds.map(function (item) {
                        return item[fatherAppId];
                    }).indexOf(association);
        ...
        insertAuthorizationLdapGroups: function (obj) {
            ...
            if (appCreateReg.actionMode !== null) {
                ...
                if ((selGroup !== '') && (selRoleVal !== '')) {
                    ...
                    $.alert({title: 'ERROR!', theme: 'light', content: appCreateReg.validations.authuser});
                }
            }
        },
        ...
        insertAssociation: function () {
            ...
            if (appCreateReg.actionMode !== null) {
                ...
                if (($('#rolesFatherApp').val() !== '') && ($('#childApps').val() !== '') && ($('#rolesChildApp').val() !== '')) {
                    ...
                    $.alert({title: 'ERROR!', theme: 'light', content: appCreateReg.validations.assocEmpty});
                }
            }
        },
        // REMOVE authorization
