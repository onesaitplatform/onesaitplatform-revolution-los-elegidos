...
angular
    ...
    .controller('HomeController', ["$scope", "$rootScope", "$routeParams", "$q", "$modal", "$location", "pipelineService", "api", "configuration", "pipelineConstant", "Analytics", "$route", "$translate", function (
        ...
        angular.extend($scope, {
            ...
            addLabelsToSelectedPipelines: function () {
                ...
                modalInstance.result.then(function (res) {
                    angular.forEach(res.successEntities, function (pipelineName) {
                        var pipeline = _.find($scope.filteredPipelines, function (p) {
                            return p.pipelineId === pipelineName;
                        });
            ...
            startPipeline: function (pipelineInfo, $event) {
                ...
                if ($rootScope.common.pipelineStatusMap[pipelineInfo.pipelineId].state !== 'RUNNING') {
                    api.pipelineAgent.startPipeline(pipelineInfo.pipelineId, 0).then(
                        function (res) {
