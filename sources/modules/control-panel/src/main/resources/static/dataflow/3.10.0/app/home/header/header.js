    ...
    .controller('HeaderController', ["$scope", "$rootScope", "$timeout", "_", "api", "$translate", "$location", "authService", "pipelineService", "pipelineConstant", "$modal", "$q", "$route", function (
        ...
        angular.extend($scope, {
            ...
            validatePipeline: function () {
                ...
                api.pipelineAgent.validatePipeline($scope.activeConfigInfo.pipelineId, $scope.edgeHttpUrl).then(
                    function (res) {
                        var defer = $q.defer();
            ...
            startPipeline: function () {
                ...
                if ($rootScope.common.pipelineStatusMap[$scope.activeConfigInfo.pipelineId].status !== 'RUNNING') {
                    ...
                    $rootScope.common.errors = [];
                    api.pipelineAgent.startPipeline($scope.activeConfigInfo.pipelineId, 0).then(
                        function (res) {
