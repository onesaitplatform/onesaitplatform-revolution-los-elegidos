            ...
            function range(name, value) {
                ...
                if (!set) {
                    ...
                    switch (name.toLowerCase()) {
                        ...
                        case 'all':
                        default:
                            return {minX, maxX, rangeX, minY, maxY, rangeY};
                    }
                }
                let // changed = false,
                    newMinX,
        ...
        Color: function (init) { // eslint-disable-line object-shorthand
            ...
            function val(name, value, context) {
                ...
                if (!set) {
                    ...
                    switch (name.toLowerCase()) {
                        case 'ahex':
                            return ColorMethods.rgbaToHex({r, g, b, a});
                        case 'hex':
                ...
                if (context !== colorMap) {
                    switch (settings.color.mode) {
                        case 'h': {
                            const sv = ui.val('sv');
                            colorMap.val('xy', {
                                x: !isNullish(sv) ? sv.s : 100,
                                y: 100 - (!isNullish(sv) ? sv.v : 100)
                            }, context);
                            break;
                        }
                        case 's':
