...
// From MDN
    ...
    JSONEditor.prototype = {
        ...
        _getDefinitions: function (schema, path) {
            ...
            path = path || '#/datos';
            if (schema.datos) {
                this.refs[path] = schema.datos;
                /* for(var i in schema.datos) {
        if(!schema.datos.hasOwnProperty(i)) continue;
        this.refs[path+i] = schema.datos[i];
        if(schema.datos[i].datos) {
          this._getDefinitions(schema.datos[i],path+i+'/datos/');
        }
      }*/
            }
        },
