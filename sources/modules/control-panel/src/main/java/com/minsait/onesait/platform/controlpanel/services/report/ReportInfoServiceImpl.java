...
public class ReportInfoServiceImpl implements ReportInfoService {
    ...
    private ReportInfoDto extractFromReport(JasperReport report) {
        ...
        if (report.getParameters() != null) {
            parameters = Arrays.stream(report.getParameters()).filter(
                    p -> p.isForPrompting() && !p.isSystemDefined()).map(
                    p -> ReportParameter.builder().name(p.getName()).description(p.getDescription()).type(
                            ReportParameterType.fromJavaType(p.getValueClass().getName())).value(
                            p.getDefaultValueExpression() != null ? p.getDefaultValueExpression().getText().replaceAll(
                                    "\"", "") : null).build()).collect(Collectors.toList());
            dataSource = Arrays.stream(report.getParameters()).filter(
                    p -> !p.isForPrompting() && p.getName().equalsIgnoreCase(JSON_DATA_SOURCE_ATT_NAME)).map(
                    parameter -> {
        ...
        if (report.getFields() != null) {
            fields = Arrays.stream(report.getFields()).map(
                    f -> ReportField.builder().name(f.getName()).description(f.getDescription()).type(
                            f.getValueClass()).build()).collect(Collectors.toList());
        }
    ...
    private byte[] generateFromReport(JasperReport jasperReport, Map<String, Object> params,
            ReportType type) throws JRException {
    ...
    private boolean hasDatasource(JasperReport report) {
        return Arrays.stream(report.getParameters()).filter(
                p -> p.getName().equalsIgnoreCase(JSON_DATA_SOURCE_ATT_NAME)).map(p -> true).findFirst().orElse(false);
