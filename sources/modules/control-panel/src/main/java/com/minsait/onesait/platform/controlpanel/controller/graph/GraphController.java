...
public class GraphController {
    ...
    @GetMapping("/getgraph")
    public @ResponseBody
    String getGraph(Model model, @RequestParam(value = "all", required = false) Boolean all) {
    ...
    public @ResponseBody
    String getGraph4Project(Model model, @PathVariable("id") String projectId) {
        ...
        arrayLinks.addAll(graphUtil.constructGraphWithOntologies(
                resources.stream().filter(r -> r instanceof Ontology).map(r -> (Ontology) r).collect(
                        Collectors.toList()), null));
        arrayLinks.addAll(graphUtil.constructGraphWithClientPlatforms(
                resources.stream().filter(r -> r instanceof ClientPlatform).map(r -> (ClientPlatform) r).collect(
                        Collectors.toList()), null));
        arrayLinks.addAll(graphUtil.constructGraphWithVisualization(
                ...
                resources.stream().filter(r -> r instanceof Dashboard).map(r -> (Dashboard) r).collect(
                        Collectors.toList()), null));
        ...
        arrayLinks.addAll(graphUtil.constructGraphWithDigitalTwins(
                resources.stream().filter(r -> r instanceof DigitalTwinDevice).map(r -> (DigitalTwinDevice) r).collect(
                        Collectors.toList()), null));
        arrayLinks.addAll(graphUtil.constructGraphWithFlows(
                resources.stream().filter(r -> r instanceof FlowDomain).map(r -> (FlowDomain) r).collect(
                        Collectors.toList()), null));
        arrayLinks.addAll(graphUtil.constructGraphWithWebProjects(webproject, null));
        arrayLinks.addAll(graphUtil.constructGraphWithNotebooks(
                resources.stream().filter(r -> r instanceof Notebook).map(r -> (Notebook) r).collect(
                        Collectors.toList()), null));
        arrayLinks.addAll(graphUtil.constructGraphWithDataFlows(
                resources.stream().filter(r -> r instanceof Pipeline).map(r -> (Pipeline) r).collect(
                        Collectors.toList()), null));
