...
public class GadgetController {
    ...
    public String list(Model uiModel, HttpServletRequest request) {
        ...
        List<GadgetTemplate> gadgetTemplate =
                this.gadgetTemplateService.findGadgetTemplateWithIdentificationAndDescription(
    ...
    public @ResponseBody
    String saveGadgetIframe(Gadget gadget, String jsonMeasures, String datasourcesMeasures) {
        Gadget g;
        try {
            gadget.setUser(userService.getUser(utils.getUserId()));
        ...
        return "{\"id\":\"" + g.getId() + "\",\"identification\":\"" + g.getIdentification() + "\",\"type\":\"" + g.getType() + "\"}";
    }
    ...
    public String createGadget(Model model, @PathVariable("gadgetId") String gadgetId) {
        ...
        model.addAttribute("dataSourceAccessType", gadgetDatasourceService.getAccessType(
                gadgetService.getGadgetMeasuresByGadgetId(utils.getUserId(), gadgetId).get(0).getDatasource().getId(),
                utils.getUserId()));
        return GADGETS_CREATE;
    }
    ...
    public @ResponseBody
    String saveUpadteGadgetIframe(@PathVariable("gadgetId") String gadgetId, Gadget gadget, String jsonMeasures,
            String datasourcesMeasures) {
        if (!gadgetService.hasUserPermission(gadgetId, utils.getUserId())) {
            return REDIRECT_ERROR;
        }
        final Gadget g;
    ...
    public @ResponseBody
    String existGadget(String identification) {
    ...
    private List<OntologyDTO> getOntologiesDTO() {
        ...
        return ontologies.stream().map(
                o -> OntologyDTO.builder().identification(o.getIdentification()).description(o.getDescription()).user(
                        o.getUser().getUserId()).build()).collect(Collectors.toList());
