...
public class DigitalTwinDeviceHelper {
    ...
    private File createFile(String path) {
        ...
        if (!file.exists()) {
            ...
            log.error(
                    "Creating project for Digital Twin Device falied, the temporary directory don't exist: " + file.getAbsolutePath());
            return null;
        }
    ...
    public File generateProject(String identificacion, Boolean compile, Boolean sensehat) {
        ...
        // Creation SRC file
        final File src = createFile(
                projectDirectory + File.separator + device.getIdentification() + File.separator + "src" + File.separator + "main");
        ...
        final File app = createFile(
                src.getAbsolutePath() + File.separator + "java" + File.separator + "digitaltwin" + File.separator +
                        "device");
        ...
        // Create logic.js
        final File fileStatic = createFile(
                src.getAbsolutePath() + File.separator + RESOURCES_STR + File.separator + "static" + File.separator + "js");

        // Create wot
        final File fileJson = createFile(
                src.getAbsolutePath() + File.separator + RESOURCES_STR + File.separator + "static" + File.separator + "json");
        ...
        for (final PropertyDigitalTwinType prop : propsDigitalTwin) {
            ...
            statusProperties.add(
                    new PropertiesDTO(prop.getName(), GeneratorJavaTypesMapper.mapPropertyName(prop.getType())));
            properties.add(new PropertiesDTO(
                    "operation" + prop.getName().substring(0, 1).toUpperCase() + prop.getName().substring(1),
                    "OperationType"));
            inits.add("setOperation" + prop.getName().substring(0, 1).toUpperCase() + prop.getName().substring(
                    1) + "(OperationType." + prop.getDirection().toUpperCase() + ");");
        ...
        try (Writer writerDeviceApplication = new FileWriter(
                app + File.separator + "DeviceApplication.java"); Writer writerDockerfile = new FileWriter(
                docker + File.separator + "Dockerfile"); Writer writerTwinStatus = new FileWriter(
                fileJava + File.separator + "DigitalTwinStatus.java"); PrintWriter outLogic = new PrintWriter(
                fileStatic + File.separator + "logic.js"); PrintWriter outWot = new PrintWriter(
                fileJson + File.separator + "wot.json"); Writer writerApplicationProperties = new FileWriter(
                src.getAbsolutePath() + File.separator + RESOURCES_STR + File.separator + "application.yml"); Writer writerPom = new FileWriter(
                projectDirectory + File.separator + device.getIdentification() + File.separator + "pom.xml"); Writer writerSwagger = new FileWriter(
                fileJava + File.separator + "SwaggerConfig.java");) {
