...
public class DashboardFIQL {
    ...
    public DashboardDTO toDashboardDTO(Dashboard dashboard, String url, String viewUrl, String categoryId,
        ...
        return DashboardDTO.builder().identification(dashboard.getIdentification()).id(dashboard.getId()).description(
                dashboard.getDescription()).user(dashboard.getUser().getUserId()).url(url + dashboard.getId()).isPublic(
                dashboard.isPublic()).category(categoryId).subcategory(subCategoryId).nGadgets(nGadgets).headerlibs(
                dashboard.getHeaderlibs()).createdAt(ft.format(dashboard.getCreatedAt())).modifiedAt(
                ft.format(dashboard.getUpdatedAt())).viewUrl(viewUrl + dashboard.getId()).dashboardAuths(
                dashAuthsDTO).build();
    }
