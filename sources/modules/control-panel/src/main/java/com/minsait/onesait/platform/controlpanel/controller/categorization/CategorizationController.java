...
public class CategorizationController {
    ...
    public String edit(Model model, @PathVariable("id") String id) {
        ...
        if (!categorization.getUser().getUserId().equals(utils.getUserId()) && !utils.getRole().equals(
    ...
    public String show(Model model, @PathVariable("id") String id) {
        final Categorization categorization = categorizationRepository.findById(id);
        User user = userService.getUser(utils.getUserId());
        if (!categorizationService.hasUserPermission(user, categorization) && !utils.getRole().equals(
                Role.Type.ROLE_ADMINISTRATOR.toString())) {
            return E403;
        }
        model.addAttribute(CATEGORIZATION, categorization);
    ...
    public String share(Model model, @PathVariable("id") String id) {
        ...
        final List<CategorizationUser> catUsers = categorizationUserRepository.findByCategorizationNotOwn(user,
                                                                                                          categorizationRepository.findById(
                                                                                                                  id));
    ...
    public ResponseEntity<String> addAuthorization(@RequestParam("id") String id, @RequestParam("user") String userId,
        ...
        final Categorization categorization = categorizationRepository.findById(id);
        if (!categorization.getUser().getUserId().equals(utils.getUserId()) || shareType.equals(
                "OWNER") && !utils.getRole().equals(Role.Type.ROLE_ADMINISTRATOR.toString())) {
    ...
    @Transactional
    @RequestMapping(value = "/getCategorizationJson", method = {RequestMethod.GET, RequestMethod.POST}, produces =
            MediaType.APPLICATION_JSON_UTF8_VALUE)
    ...
    @Transactional
    @RequestMapping(value = "/getOntologies", method = RequestMethod.GET, produces =
            MediaType.APPLICATION_JSON_UTF8_VALUE)
    ...
    public ResponseEntity<String> getDevices() {
        try {
            JSONObject list = new JSONObject();
            List<ClientPlatform> devices = clientPlatformService.getAllClientPlatformByCriteria(utils.getUserId(), null,
                                                                                                null);
    ...
    @Transactional
    @RequestMapping(value = "/getDashboards", method = RequestMethod.GET, produces =
            MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<String> getDashboards() {
        try {
            JSONObject list = new JSONObject();
            List<DashboardDTO> dashboards = dashboardService.findDashboardWithIdentificationAndDescription(null, null,
                                                                                                           utils.getUserId());
    ...
    @Transactional
    @RequestMapping(value = "/getDataflows", method = RequestMethod.GET, produces =
            MediaType.APPLICATION_JSON_UTF8_VALUE)
    ...
    @Transactional
    @RequestMapping(value = "/getNotebooks", method = RequestMethod.GET, produces =
            MediaType.APPLICATION_JSON_UTF8_VALUE)
    ...
    private String getIdentification(String id, String type) {
        ...
        switch (type) {
            ...
            case "flows":
                try {
                    if (flowRepository.findByIdentification(id) != null && (flowRepository.findByIdentification(
                            id).getUser().getUserId().equals(utils.getUserId()) || utils.getRole().equals(
                            Role.Type.ROLE_ADMINISTRATOR.toString()))) {
            ...
            case "devices":
                try {
                    List<ClientPlatform> devices = clientPlatformService.getAllClientPlatformByCriteria(
                            utils.getUserId(), null, null);
            ...
            case "dashboards":
                try {
                    ...
                    List<DashboardDTO> dashboards = dashboardService.findDashboardWithIdentificationAndDescription(null,
                                                                                                                   null,
                                                                                                                   utils.getUserId());
