...
public class LayersRestImpl implements LayersRest {
    ...
    public ResponseEntity<?> getLayerData(HttpServletRequest request) {
        ...
        try {
            ...
            if (layer != null && (layer.getUser().equals(user) || user.getRole().getId().equals(
                ...
                if (jsonArray != null) {
                    for (Integer i = 0; i < jsonArray.length(); i++) {
                        ...
                        if ((layer.getGeometryType().equalsIgnoreCase(
                                GeometryType.POINT.getName())) || layer.getGeometryType().equalsIgnoreCase(RASTER)) {
                                ...
                                for (int j = 0; j < splitAux.length - 1; j++) {
                                    if (j + 1 == splitAux.length - 1) {
                                        try {
                                            ...
                                            value = rootObjectArray.get(
                                                    Integer.parseInt(splitAux[splitAux.length - 1]));
            ...
            for (Entry<String, String[]> entry : paramsMap.entrySet()) {
                ...
                if (jsonParams.length() != 0) {
                    ...
                    if (obj.getString(PARAM).equals(param)) {
                        ...
                        } else if (type.equals(BOOLEAN) && (!value.equalsIgnoreCase("true") && !value.equalsIgnoreCase(
                                "false"))) {
                            throw new BadRequestException(WRONG_PARAMETER_TYPE + param);
                        }
        ...
        try {
            ...
            for (SelectItem item : selectItems) {
                if (!item.toString().equals("c") && !item.toString().equals("*")) {
                    ...
                    isQuery = false;
                    Map<String, String> mapFields = ontologyService.getOntologyFields(
                            layer.getOntology().getIdentification(), userId);
        ...
        if (ontologyService.hasUserPermissionForQuery(userId, ontologyIdentification)) {
            ...
            if (query == null) {
                queryResult = queryToolService.querySQLAsJson(userId, ontologyIdentification,
                                                              "select _id,c from " + ontologyIdentification + " as c",
                                                              0);
            } else {
