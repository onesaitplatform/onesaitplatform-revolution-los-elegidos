    ...
    public void populateAppUpdate(Model model, App app, User sessionUser, String ldapBaseDn, boolean ldapActive) {
        ...
        for (final App appToChoose : appService.getAllApps()) {
            if (appToChoose.getChildApps() != null && appToChoose.getChildApps().contains(app)) {
                ...
                // apps hijas
                appsToChoose = appService.getAppsByUser(sessionUser.getUserId(), null).stream().filter(
                        appToSelect -> ((!appToSelect.getAppId().equals(
                                app.getAppId())) && (appToSelect.getChildApps() == null || appToSelect.getChildApps().isEmpty()))).collect(
                        Collectors.toList());
            }
        }
        if (app.getProject() != null) {
            ...
            model.addAttribute("projects", projectService.getAllProjects().stream().filter(
                    p -> p.getApp() == null && CollectionUtils.isEmpty(p.getUsers())).collect(Collectors.toList()));
        }
    }
