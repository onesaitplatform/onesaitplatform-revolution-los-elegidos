...
public class UserController {
    ...
    @Value("${onesaitplatform.user.reset.validation" +
            ".url:http://localhost:18000/controlpanel/users/validateResetPassword/}")
    ...
    public @ResponseBody
    Map<String, String> deleteSimpleData(RedirectAttributes redirect, @RequestBody OntologyRemoveRevokeDto data) {
    ...
    public @ResponseBody
    Map<String, String> revokeSimpleData(RedirectAttributes redirect, @RequestBody OntologyRemoveRevokeDto data) {
        ...
        try {
            if ((!newPass.isEmpty()) && (!repeatPass.isEmpty())) {
                if (newPass.equals(repeatPass) && utils.paswordValidation(newPass)) {
                    ...
                    if (utils.isAdministrator()) {
                        final String defaultMessage = "Your password has been changed. Your new password for onesait " +
                                "Platform: ";
                        final String emailMessage = utils.getMessage("user.update.password.email",
                                                                     defaultMessage).concat(" " + newPass);
                        try {
    ...
    public String create(@Valid User user, BindingResult bindingResult, RedirectAttributes redirect,
        ...
        try {
            ...
            if (!userService.emailExists(user)) {
                if ((!newPass.isEmpty()) && (!repeatPass.isEmpty()) && newPass.equals(
                        repeatPass) && utils.paswordValidation(newPass)) {
                    user.setPassword(newPass);
            ...
            if (!userService.emailExists(user)) {
                ...
                try {
                    ...
                    if (!inFlight) {
                        ...
                        final String defaultMessage = "To complete your registry in Onesait Plaform, please follow " +
                                "this link:";
    ...
    public String validateResetPassword(@PathVariable(name = "uuid") String uuid,
        ...
        try {
            ...
            if (user != null) {
                final String newPassword = UUID.randomUUID().toString().toUpperCase().substring(0,
                                                                                                2) + UUID.randomUUID().toString().substring(
                        0, 10) + "$";
