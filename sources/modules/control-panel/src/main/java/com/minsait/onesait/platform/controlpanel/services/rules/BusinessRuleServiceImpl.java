...
public class BusinessRuleServiceImpl implements BusinessRuleService {
    ...
    private void publishHzRuleNotification(String identification, User user, String drl) {
        final DroolsRule rule = droolsRuleService.getRule(identification);
        final HazelcastRuleObject ruleObj = HazelcastRuleObject.builder().identification(identification).userId(
                rule == null ? user.getUserId() : rule.getUser().getUserId()).DRL(drl).build();
    ...
    private void publishHzRuleNotification(String identification, String drl) {
        final DroolsRule rule = droolsRuleService.getRule(identification);
        final HazelcastRuleObject ruleObj = HazelcastRuleObject.builder().identification(identification).userId(
                rule.getUser().getUserId()).DRL(drl).build();
        ruleObj.toJson().ifPresent(s -> topicRules.publish(s));
    }

    private void publishAndHandleHzNotification(String identification,
            String drl) throws InterruptedException, ExecutionException, TimeoutException {
