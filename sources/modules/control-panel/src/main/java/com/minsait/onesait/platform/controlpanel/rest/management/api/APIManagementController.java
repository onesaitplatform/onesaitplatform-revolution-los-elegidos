...
public class APIManagementController {
    ...
    public ResponseEntity<?> authorize(
        ...
        final List<com.minsait.onesait.platform.config.model.Api> apis = apiManagerService.loadAPISByFilter(apiId, "",
                                                                                                            loggedUser,
                                                                                                            loggedUser).stream().filter(
                a -> a.getUser().getUserId().equals(loggedUser) && a.getIdentification().equals(apiId)).collect(
                Collectors.toList());
    ...
    public ResponseEntity<?> deauthorize(
        ...
        final List<com.minsait.onesait.platform.config.model.Api> apis = apiManagerService.loadAPISByFilter(apiId, "",
                ...
                Collectors.toList());
        if (!apis.isEmpty()) {
            final List<UserApi> userApi = userApiRepository.findByUser(userService.getUser(userId)).stream().filter(
                    ua -> ua.getApi().getIdentification().equals(apiId)).collect(Collectors.toList());
    ...
    public ResponseEntity<String> deleteApiToken(
        ...
        final UserToken tokenOj = userTokenRepository.findByUserAndToken(userService.getUser(utils.getUserId()), token);
        if (tokenOj == null)
    ...
    public ResponseEntity<?> getApiList() {
        ...
        if (!apis.isEmpty()) {
            ...
            for (com.minsait.onesait.platform.config.model.Api api : apis) {
                List<com.minsait.onesait.platform.config.model.ApiOperation> apiops =
                        apiOperationRepository.findByApiIdOrderByOperationDesc(
                        api.getId());
    ...
    public ResponseEntity<?> getApiByIdentification(
        ...
        if (api != null) {
            if (apiManagerService.hasUserAccess(api.getId(), utils.getUserId())) {
                final List<com.minsait.onesait.platform.config.model.ApiOperation> apiops =
                        apiOperationRepository.findByApiIdOrderByOperationDesc(
                        api.getId());
    ...
    public ResponseEntity<String> deleteApi(
            @ApiParam(value = "Api id", required = true) @PathVariable("id") String apiId) {
        try {
    ...
    public ResponseEntity<?> updateApi(@ApiParam(value = "APIBody", required = true) @Valid @RequestBody ApiDTO apiBody,
        ...
        List<com.minsait.onesait.platform.config.model.Api> apimemory = apiManagerService.loadAPISByFilter(
                apiBody.getIdentification(), "", "", utils.getUserId());
    ...
    private List<com.minsait.onesait.platform.config.model.ApiOperation> toAPIOperations(
        ...
        for (ApiOperationDTO apiopdto : apiopsDTO) {
            com.minsait.onesait.platform.config.model.ApiOperation apiop =
                    new com.minsait.onesait.platform.config.model.ApiOperation();
            apiop.setApi(api);
    ...
    public ResponseEntity<?> exportApiByIdentification(
        ...
        if (api != null) {
            if (apiManagerService.hasUserAccess(api.getId(), utils.getUserId())) {
                final List<com.minsait.onesait.platform.config.model.ApiOperation> apiops = apiOperationRepository.findByApiIdOrderByOperationDesc(
                        api.getId());
                final List<UserApi> usersapi = userApiRepository.findByApiId(api.getId());
                final ApiDTO apidto = new ApiDTO(api, apiops, usersapi);
