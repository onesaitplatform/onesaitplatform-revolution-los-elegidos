...
public class ClientPlatformManagementController {
    ...
    @ApiResponses({@ApiResponse(code = 200, message = "OK", response = ClientPlatformDTO.class), @ApiResponse(code =
    ...
    private ClientPlatformDTO parseClientPlatform(ClientPlatform clientPlatform) {
        ...
        return ClientPlatformDTO.builder().user(clientPlatform.getUser().getUserId()).description(
                clientPlatform.getDescription()).identification(clientPlatform.getIdentification()).metadata(
                clientPlatform.getMetadata()).ontologies(ontologies).tokens(tokens).build();
