...
public class ApiManagementControllerTest {
    ...
    public void setUp() throws Exception {
        ...
        String response = mvc.perform(
                MockMvcRequestBuilders.post("/api-ops/login").contentType(MediaType.APPLICATION_JSON).content(
                        "{\"password\":\"Changed!\",\"username\":\"developer\"}")).andReturn().getResponse().getContentAsString();
    ...
    public void user_triesToChangeItsApiAuthorizations_andGetsOk() throws Exception {
        mvc.perform(MockMvcRequestBuilders.post(
                "/management/authorize/api/" + this.apiDeveloper.getId() + "/user/analytics").header("Authorization",
                                                                                                     this.oAuthHeader)).andExpect(
                MockMvcResultMatchers.status().is2xxSuccessful());
        mvc.perform(MockMvcRequestBuilders.post(
                "/management/deauthorize/api/" + this.apiDeveloper.getId() + "/user/analytics").header("Authorization",
                                                                                                       this.oAuthHeader)).andExpect(
                MockMvcResultMatchers.status().is2xxSuccessful());
    ...
    public void user_triesToChangeOthersApiAuthorizations_andGetsKo() throws Exception {
        mvc.perform(MockMvcRequestBuilders.post(
                "/management/authorize/api/" + this.apiDataViewer.getId() + "/user/analytics").header("Authorization",
                                                                                                      this.oAuthHeader)).andExpect(
