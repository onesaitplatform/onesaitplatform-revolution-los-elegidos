...
public class OntologyControllerTest {
    ...
    public void given_OneOntology_When_AnIncorrectIdIsSentToDeleteOne_TheViewIsRedirectToSamePage() throws Exception {
        ...
        mockMvc.perform(delete("/ontologies/" + ontology.getId())).andExpect(
                redirectedUrl("/ontologies/update/" + ontology.getId()));
    }
    ...
    public void given_OneOntology_When_OneUserWithoutAuthorizationWantsToUpdateIt_Then_TheViewIsRedirectToCreate() throws Exception {
        ...
        given(ontologyService.getOntologyById(ontology.getId(), sessionUserId)).willThrow(
                new OntologyServiceException("The user is not authorizated"));
    ...
    public void given_OneOntology_WhenCorrectParementersAreSentToUpdate_TheCreateWizardViewIsShowedWithTheCorrectParameters() throws Exception {
        ...
        mockMvc.perform(get("/ontologies/update/" + ontology.getId())).andExpect(status().isOk()).andExpect(
                view().name("ontologies/createwizard")).andExpect(model().attribute("ontology", ontology)).andExpect(
    ...
    public void given_OneOntology_When_OneUserWithoutAuthorizationWantsToViewDetails_Then_TheListViewIsServed() throws Exception {
        ...
        given(ontologyService.getOntologyById(ontology.getId(), sessionUserId)).willThrow(
                new OntologyServiceException("The user is not authorized"));
    ...
    public void given_OneOntology_When_CorrectParametersAreProvided_Then_TheShowViewIsServedWithTheCorrectAttributes() throws Exception {
        ...
        mockMvc.perform(get("/ontologies/show/" + ontology.getId())).andExpect(status().isOk()).andExpect(
                view().name("ontologies/show")).andExpect(model().attribute("ontology", ontology)).andExpect(
                model().attribute("users", users))
                // authorizations is serialized using OntologyUserAccessDTO, to check the
                // content of
                // this attribute, it would be necessary to implement a custom
    ...
    public void given_OneOntology_When_OneNotAuthorizedUserWantsToCreateAUserAccess_Then_TheUserAccessIsNotCreatedAndABadRequestIsResponsed() throws Exception {
        ...
        mockMvc.perform(post("/ontologies/authorization").param("accesstype",
                                                                access.getOntologyUserAccessType().getName()).param(
                "ontology", access.getOntology().getId()).param("user", access.getUser().getUserId())).andExpect(
                status().isBadRequest());
    }
    ...
    public void given_OneOntologyWithUserAccess_When_CorrectParamentersAreSentToDeleteTheUserAccess_Then_TheUserAccessIsDeletedAndAStatusIsResponsed() throws Exception {
        ...
        mockMvc.perform(post("/ontologies/authorization/delete").param("id", access.getId())).andExpect(
    ...
    public void given_OneOnotologyWithUserAccess_When_OneUserWithoutAuthorizationWantsToDeleteTheUserAccess_Then_TheUserAccessIsNotDeletedAndABadRequestIsResponsed() throws Exception {
        ...
        doThrow(new OntologyServiceException("The user is not authorized")).when(
                ontologyService).deleteOntologyUserAccess(access.getId(), sessionUserId);
    ...
    public void given_OneOntologyWithUserAccess_When_CorrectParametersAreSentToUpdate_Then_TheOntologyUserAccessIsUpdatedAndTheNewValuesAreReturnedAsJSON() throws Exception {
        ...
        mockMvc.perform(post("/ontologies/authorization/update").param("id", accessOld.getId()).param("accesstype",
                                                                                                      accessOld.getOntologyUserAccessType().getName())).andExpect(
                status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(
                jsonPath("$.id", is(accessNew.getId()))).andExpect(
                jsonPath("$.userId", is(accessNew.getUser().getUserId()))).andExpect(
                jsonPath("$.typeName", is(accessNew.getOntologyUserAccessType().getName())));
    }
    ...
    public void given_OneOntologyWithUserAccess_When_OneUserWithAuthorizationsWantsToUpdateTheUserAccess_Then_TheUpdateIsNotPerformedAndABadRequestIsResponsed() throws Exception {
        ...
        doThrow(new OntologyServiceException("The user is not authorizated")).when(
                ontologyService).updateOntologyUserAccess(access.getUser().getUserId(), "QUERY", sessionUserId);

        mockMvc.perform(post("/ontologies/authorization/update").param("id", access.getId()).param("accesstype",
                                                                                                   access.getOntologyUserAccessType().getName())).andExpect(
                status().isBadRequest());
    }
    ...
    public void given_OneOntologyWithTwoUserAccesses_When_CorrectParametersAreSentToListUserAccesses_Then_TheTwoUserAccessesAreResponsedAsAJSONArray() throws Exception {
        ...
        mockMvc.perform(get("/ontologies/authorization").param("id", ontology.getId())).andExpect(
                status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(
                jsonPath("$", hasSize(2))).andExpect(jsonPath("$[0].id", is(accesses.get(0).getId()))).andExpect(
                jsonPath("$[0].userId", is(accesses.get(0).getUser().getUserId()))).andExpect(
                jsonPath("$[0].typeName", is(accesses.get(0).getOntologyUserAccessType().getName()))).andExpect(
                jsonPath("$[0].ontologyId", is(accesses.get(0).getOntology().getId()))).andExpect(
                jsonPath("$[1].id", is(accesses.get(1).getId()))).andExpect(
                jsonPath("$[1].userId", is(accesses.get(1).getUser().getUserId()))).andExpect(
                jsonPath("$[1].typeName", is(accesses.get(1).getOntologyUserAccessType().getName()))).andExpect(
                jsonPath("$[1].ontologyId", is(accesses.get(1).getOntology().getId())));
    }
