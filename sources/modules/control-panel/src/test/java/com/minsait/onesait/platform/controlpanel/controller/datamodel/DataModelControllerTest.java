...
public class DataModelControllerTest {
    ...
    @Test
    public void given_ThereAreTwoDataModels_When_TheyHaveToBeListedWithoutFilters_Then_AllOfThemAreListed() throws Exception {
        ...
        mockMvc.perform(
                get("/datamodels/list").param("dataModelId", "").param("name", "").param("description", "")).andExpect(
                view().name("datamodels/list")).andExpect(model().attribute("dataModels", allDataModels));
    ...
    public void given_ThereIsOneDataModel_When_TheCorrectIdIsProvidedToShow_Then_TheDetailsOfTheDataModelIsShown() throws Exception {
        ...
        mockMvc.perform(get("/datamodels/show/" + id)).andExpect(view().name("datamodels/show")).andExpect(
                model().attribute("dataModel", dm));
