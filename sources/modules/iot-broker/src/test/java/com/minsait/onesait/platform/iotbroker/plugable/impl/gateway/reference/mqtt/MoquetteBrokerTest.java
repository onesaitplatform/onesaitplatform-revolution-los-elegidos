...
public class MoquetteBrokerTest {
    ...
    public void given_OneMqttClientConnection_When_ACommandIsTrigger_Then_ItGetsTheCommand() throws Exception {
        ...
        final SSAPMessage<SSAPBodyIndicationMessage> responseCommand = SSAPJsonParser.getInstance().deserialize(
                responseCommandStr);
    ...
    public void given_OneMqttClientConnection_When_ItSubscribesToATopicAndSendsMessage_Then_ItGetsTheMessage() throws Exception {
        ...
        resultAction = mockMvc.perform(MockMvcRequestBuilders.post(URL_ADVICE_PATH).accept(
                org.springframework.http.MediaType.APPLICATION_JSON).content(content).contentType(
                org.springframework.http.MediaType.APPLICATION_JSON));

        resultAction.andExpect(status().is2xxSuccessful());
        final OperationResultModel result = mapper.readValue(
                resultAction.andReturn().getResponse().getContentAsString(), OperationResultModel.class);

        // Waits to recieve indication
        final String indicationStr = completableFutureIndication.get(5, TimeUnit.SECONDS);
        final SSAPMessage<SSAPBodyIndicationMessage> indication = SSAPJsonParser.getInstance().deserialize(
                indicationStr);
