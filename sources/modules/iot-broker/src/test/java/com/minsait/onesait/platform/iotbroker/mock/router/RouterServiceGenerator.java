...
public class RouterServiceGenerator {
    ...
    public static NotificationCompositeModel generateNotificationCompositeModel(String subscriptionId, Person subject,
        ...
        model.getNotificationModel().setOperationModel(
                OperationModel.builder(Person.class.getSimpleName(), OperationModel.OperationType.QUERY,
                                       session.getUserID(), Source.IOTBROKER).body(
                        mapper.writeValueAsString(subject)).deviceTemplate(
                        mapper.writeValueAsString(subject)).queryType(QueryType.NATIVE).objectId(
