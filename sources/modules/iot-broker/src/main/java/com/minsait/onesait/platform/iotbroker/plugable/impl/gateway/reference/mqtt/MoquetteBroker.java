...
@ConditionalOnProperty(prefix = "onesaitplatform.iotbroker.plugable.gateway.moquette", name = "enable", havingValue =
        "true")
@Slf4j
@Component
public class MoquetteBroker {
    ...
    class PublisherListener extends AbstractInterceptHandler {
        ...
        public void onPublish(InterceptPublishMessage msg) {
            ...
            final MqttPublishMessage message = MqttMessageBuilders.publish().topicName(
                    outboundTopic + "/" + msg.getClientID()).retained(false).qos(MqttQoS.EXACTLY_ONCE).payload(
                    Unpooled.copiedBuffer(response.getBytes())).build();
    ...
    public void init() {
        try {
            ...
            subscriptor.addSubscriptionListener("mqtt_gateway", s -> {
                ...
                final MqttPublishMessage message = MqttMessageBuilders.publish().topicName(
                        subscriptionTopic + "/" + s.getSessionKey()).retained(false).qos(MqttQoS.EXACTLY_ONCE).payload(
                        Unpooled.copiedBuffer(playload.getBytes())).build();
            ...
            subscriptor.addCommandListener("mqtt_gateway",
                                           ...
                                           s -> {
                                               ...
                                               final MqttPublishMessage message =
                                                       MqttMessageBuilders.publish().topicName(
                                                       commandTopic + "/" + s.getSessionKey()).retained(false).qos(
                                                       MqttQoS.EXACTLY_ONCE).payload(
                                                       Unpooled.copiedBuffer(playload.getBytes())).build();

                                               MoquetteBroker.this.getServer().internalPublish(message,
                                                                                               s.getSessionKey());
                                               return null;
                                           });
