...
public class InsertProcessor implements MessageTypeProcessor {
    ...
    public SSAPMessage<SSAPBodyReturnMessage> process(SSAPMessage<? extends SSAPBodyMessage> message) {
        @SuppressWarnings("unchecked") final SSAPMessage<SSAPBodyInsertMessage> insertMessage =
                (SSAPMessage<SSAPBodyInsertMessage>) message;
        ...
        final OperationModel model = OperationModel.builder(insertMessage.getBody().getOntology(), OperationType.POST,
                                                            user, Source.IOTBROKER).body(
                insertMessage.getBody().getData().toString()).queryType(QueryType.NATIVE).deviceTemplate(
                deviceTemplate).device(device).clientSession(insertMessage.getSessionKey()).clientConnection(
                "").build();
    ...
    private JsonNode buildBrokerResponse(String routerResponse) throws IOException {
        ...
        if (type == ComplexWriteResultType.BULK) {
            final MultiDocumentOperationResult multidocument = MultiDocumentOperationResult.fromJSONObject(
                    obj.getJSONObject(InsertResult.DATA_PROPERTY));
