...
public class PlainServerCallbackHandler implements AuthenticateCallbackHandler {
    ...
    private String join(String deviceId, String token) throws IOException {
        ...
        query.append(String.format("token=%s", URLEncoder.encode(token, charset))).append("&").append(
                String.format("clientPlatform=%s", URLEncoder.encode(deviceId, charset))).append("&").append(
                String.format("clientPlatformId=%s", URLEncoder.encode(deviceId, charset)));
