...
public class ConfigDBDetailsService implements UserDetailsService {
    ...
    private UserDetails toUserDetails(User userObject) {
        return org.springframework.security.core.userdetails.User.withUsername(userObject.getUserId()).password(
                userObject.getPassword()).authorities(new SimpleGrantedAuthority(userObject.getRole().getId())).build();
    }
