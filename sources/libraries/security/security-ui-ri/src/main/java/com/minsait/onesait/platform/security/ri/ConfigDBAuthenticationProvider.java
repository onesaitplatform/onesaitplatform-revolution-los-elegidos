...
@ConditionalOnExpression("'${onesaitplatform.authentication.provider}' == 'cas' or '${onesaitplatform.authentication" +
        ".provider}' == 'configdb'")
...
public class ConfigDBAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) {
        ...
        if (tfaEnabled && role.equals(
                Role.Type.ROLE_ADMINISTRATOR.name()) && authentication.getDetails() != null && authentication.getDetails() instanceof WebAuthenticationDetails)
    ...
    private void publishFailureCredentials(User user, Authentication authentication) {
        ...
        applicationEventPublisher.publishEvent(new AuthenticationFailureBadCredentialsEvent(auth,
                                                                                            new BadCredentialsException(
                                                                                                    "Authentication failed. Password incorrect for " + user.getUserId())));
    }
