...
public class OPResourceRepositoryTest {
    ...
    public void findAnyOntologies() {
        final List<OPResource> resources = resourceRepository.findAll();
        final List<Ontology> ontologies = resources.stream().filter(r -> r instanceof Ontology).map(
                r -> (Ontology) r).collect(Collectors.toList());
    ...
    public void createProject_thenAssignResource_thenSaveToDB_thenDeleteResourceFromProject_andAssertResourceIsNotDeleted() {
        ...
        project.getProjectResourceAccesses().add(
                ProjectResourceAccess.builder().access(ResourceAccessType.MANAGE).user(user).project(project).resource(
                        resource).build());
    ...
    public void createProject_andAssignAllResources_thenDelete() {
        ...
        resourceRepository.findAll().stream().forEach(r -> accesses.add(
                ProjectResourceAccess.builder().user(user).access(ResourceAccessType.MANAGE).resource(r).project(
                        project).build()));
    ...
    public void findByIdentificationLike() {
        ...
        final OPResource ontology = resources.stream().filter(r -> (r instanceof Ontology)).filter(
                r -> r.getIdentification().equals("Ticket")).findFirst().orElse(null);
        Assert.assertNotNull(ontology);
        final OPResource client = resources.stream().filter(r -> (r instanceof ClientPlatform)).filter(
                r -> r.getIdentification().contains("Ticketing")).findFirst().orElse(null);
