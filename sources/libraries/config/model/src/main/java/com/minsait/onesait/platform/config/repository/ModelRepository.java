...
public interface ModelRepository extends JpaRepository<Model, Long> {
    ...
    @Query("SELECT o FROM Model AS o WHERE ((o.notebook.id IN (SELECT nua.notebook FROM NotebookUserAccess as nua " +
            "where nua.user=:user)) OR " + "(o.notebook.id IN (SELECT nb.id FROM Notebook as nb where nb.user=:user)))")
