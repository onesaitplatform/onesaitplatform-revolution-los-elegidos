...
public interface GadgetRepository extends JpaRepository<Gadget, String> {
    ...
    @Query("SELECT g " + "FROM Gadget AS g " + "WHERE ( g.identification=:identification)  ORDER BY g.identification " +
            "ASC")
    List<Gadget> existByIdentification(@Param("identification") String identification);

    @Query("SELECT g " + "FROM Gadget AS g " + "WHERE (g.user=:user AND g.identification=:identification)  ORDER BY g" +
            ".identification ASC")
