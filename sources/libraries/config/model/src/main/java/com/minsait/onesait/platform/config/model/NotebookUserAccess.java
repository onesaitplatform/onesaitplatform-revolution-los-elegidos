...
@Entity
@Table(name = "NOTEBOOK_USER_ACCESS", uniqueConstraints = {@UniqueConstraint(columnNames = {"NOTEBOOK_ID", "USER_ID"})})
@Configurable
public class NotebookUserAccess extends AuditableEntityWithUUID {
    ...
    public boolean equals(Object o) {
        if (this == o)
            return true;
        if (!(o instanceof NotebookUserAccess))
            return false;
        NotebookUserAccess that = (NotebookUserAccess) o;
        return getNotebookUserAccessType() != null && getNotebookUserAccessType().equals(
                that.getNotebookUserAccessType()) && getNotebook() != null && getNotebook().equals(
                that.getNotebook()) && getUser() != null && getUser().equals(that.getUser());
    }
