...
public interface DashboardRepository extends JpaRepository<Dashboard, String> {
    ...
    @Query("SELECT o " + "FROM Dashboard AS o " + "WHERE (o.isPublic=TRUE OR " + "o.user=:user OR " + "o.id IN " +
            "(SELECT uo.dashboard.id " + "FROM DashboardUserAccess AS uo " + "WHERE uo.user=:user)) AND " + "(o" +
            ".identification like %:identification% AND o.description like %:description%) ORDER BY o.identification " +
            "ASC")
    ...
    @Query("SELECT o FROM Dashboard AS o  WHERE (o.isPublic=TRUE OR o.user=:user OR " + "o.id IN (SELECT uo.dashboard" +
            ".id  FROM DashboardUserAccess AS uo  WHERE uo.user=:user)) " + " ORDER BY o.identification ASC")
    List<Dashboard> findByUserPermissionOrderByIdentificationAsc(@Param("user") User user);

    @Query("SELECT o FROM Dashboard AS o  WHERE (o.isPublic=TRUE OR o.user=:user OR " + "o.id IN (SELECT uo.dashboard" +
            ".id  FROM DashboardUserAccess AS uo  WHERE uo.user=:user)) " + " ORDER BY o.identification DESC")
    List<Dashboard> findByUserPermissionOrderByIdentificationDesc(@Param("user") User user);

    @Query("SELECT o FROM Dashboard AS o  WHERE (o.isPublic=TRUE OR o.user=:user OR " + "o.id IN (SELECT uo.dashboard" +
            ".id  FROM DashboardUserAccess AS uo  WHERE uo.user=:user)) " + " ORDER BY o.createdAt ASC")
    List<Dashboard> findByUserPermissionOrderByCreatedAtAsc(@Param("user") User user);

    @Query("SELECT o FROM Dashboard AS o  WHERE (o.isPublic=TRUE OR o.user=:user OR " + "o.id IN (SELECT uo.dashboard" +
            ".id  FROM DashboardUserAccess AS uo  WHERE uo.user=:user)) " + " ORDER BY o.createdAt DESC")
    List<Dashboard> findByUserPermissionOrderByCreatedAtDesc(@Param("user") User user);

    @Query("SELECT o FROM Dashboard AS o  WHERE (o.isPublic=TRUE OR o.user=:user OR " + "o.id IN (SELECT uo.dashboard" +
            ".id  FROM DashboardUserAccess AS uo  WHERE uo.user=:user)) " + " ORDER BY o.updatedAt ASC")
