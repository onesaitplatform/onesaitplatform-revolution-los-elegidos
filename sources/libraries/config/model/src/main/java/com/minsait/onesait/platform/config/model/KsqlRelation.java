...
@Configurable
@Entity
@Table(name = "KSQL_RELATION", uniqueConstraints = {@UniqueConstraint(columnNames = {"KSQL_FLOW_ID",
        "KSQL_RESOURCE_ID"})})
public class KsqlRelation extends AuditableEntityWithUUID {
    ...
    @JoinTable(name = "KSQL_PREDECESOR_RELATION", joinColumns = @JoinColumn(name = "KSQL_PREDECESSOR_ID"),
            inverseJoinColumns = @JoinColumn(name = "KSQL_SUCESSOR_ID"))
    @OnDelete(action = OnDeleteAction.NO_ACTION)
    @Getter
    @Setter
