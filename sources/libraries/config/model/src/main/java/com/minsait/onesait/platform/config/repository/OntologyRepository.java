...
public interface OntologyRepository extends JpaRepository<Ontology, String> {
    ...
    @Cacheable(cacheNames = "OntologyRepositoryByUser", unless = "#result==null or #result.size()==0", key = "#p0" +
            ".userId")
    List<Ontology> findByUser(User user);

    @Cacheable(cacheNames = "OntologyRepositoryByUserActiveTrue", unless = "#result==null or #result.size()==0", key
            = "#p0.userId")
    ...
    @Query("SELECT o FROM Ontology AS o WHERE (o.user=:user OR o.isPublic=TRUE OR o.id IN (SELECT uo.ontology.id FROM" +
            " OntologyUserAccess AS uo WHERE uo.user=:user)) AND o.active=true ORDER BY o.identification ASC")
    List<Ontology> findByUserAndOntologyUserAccessAndAllPermissions(@Param("user") User user);

    @Query("SELECT o FROM Ontology AS o WHERE (o.user=:user OR o.isPublic=TRUE OR o.id IN (SELECT uo.ontology.id FROM" +
            " OntologyUserAccess AS uo WHERE uo.user=:user)) ORDER BY o.identification ASC")
    List<Ontology> findByUserAndAccess(@Param("user") User user);

    @Query("SELECT o " + "FROM Ontology AS o " + "WHERE (o.isPublic=TRUE OR " + "o.user=:user OR " + "o.id IN (SELECT" +
            " uo.ontology.id " + "FROM OntologyUserAccess AS uo " + "WHERE uo.user=:user)) AND " + "(o.identification" +
            " like %:identification% AND o.description like %:description%) ORDER BY o.identification ASC")
    ...
    @Query("SELECT o " + "FROM Ontology AS o " + "WHERE (o.identification like %:identification% AND o.description " +
            "like %:description%) ORDER BY o.identification ASC")
    List<Ontology> findByIdentificationLikeAndDescriptionLike(@Param("identification") String identification,
            @Param("description") String description);

    @Query("SELECT o " + "FROM Ontology AS o " + "WHERE (o.isPublic=TRUE OR " + "o.user=:user OR " + "o.id IN (SELECT" +
            " uo.ontology.id " + "FROM OntologyUserAccess AS uo " + "WHERE uo.user=:user)) AND " + "(o.identification" +
            " like %:identification% AND o.description like %:description%) AND o.active=true ORDER BY o" +
            ".identification ASC")
    List<Ontology> findByUserAndPermissionsANDIdentificationContainingAndDescriptionContaining(@Param("user") User user,
            @Param("identification") String identification, @Param("description") String description);

    @Query("SELECT o FROM Ontology AS o WHERE o.isPublic=TRUE OR o.user=:user OR o.id IN (SELECT uo.ontology.id FROM " +
            "OntologyUserAccess AS uo WHERE uo.user=:user AND (uo.ontologyUserAccessType='ALL' OR uo" +
            ".ontologyUserAccessType='QUERY')) AND o.active=true ORDER BY o.identification ASC")
    List<Ontology> findByUserAndOntologyUserAccessAndPermissionsQuery(@Param("user") User user);

    @Query("SELECT o FROM Ontology AS o WHERE o.user=:user OR o.id IN (SELECT uo.ontology.id FROM OntologyUserAccess " +
            "AS uo WHERE uo.user=:user AND (uo.ontologyUserAccessType='ALL' OR uo.ontologyUserAccessType='INSERT')) " +
            "AND o.active=true ORDER BY o.identification ASC")
    ...
    @Query("SELECT o FROM Ontology AS o WHERE o.user=:user AND o.active=true AND o.dataModel=:datamodel ORDER BY o" +
            ".identification ASC")
