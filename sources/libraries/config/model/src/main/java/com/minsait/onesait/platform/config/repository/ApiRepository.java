...
public interface ApiRepository extends JpaRepository<Api, String> {
    ...
    @Query("SELECT a FROM Api AS a WHERE (a.user.userId = :userId OR a.identification LIKE %:apiId% OR a.state = " +
            ":state)")
    List<Api> findApisByIdentificationOrStateOrUser(@Param("apiId") String apiId, @Param("state") ApiStates state,
            @Param("userId") String userId);

    @Query("SELECT a FROM Api AS a WHERE (a.user.userId LIKE %:userId% AND (a.identification LIKE %:apiId% OR a.state" +
            " LIKE %:state%)) AND a.isPublic IS true")
    ...
    @Query("SELECT a FROM Api as a WHERE a.isPublic = false AND (a.state = 'PUBLISHED' or a.state = 'DEVELOPMENT') " +
            "ORDER BY a.identification asc")
    List<Api> findApisNotPublicAndPublishedOrDevelopment();

    @Query("SELECT a FROM Api as a WHERE a.user.userId = :userId AND a.isPublic = false AND (a.state = 'PUBLISHED' or" +
            " a.state = 'DEVELOPMENT') ORDER BY a.identification asc")
    List<Api> findApisByUserNotPublicAndPublishedOrDevelopment(@Param("userId") String userId);

    @Query("SELECT a FROM Api as a WHERE (((:userloggedRole = 'ROLE_ADMINISTRATOR') OR (a.user.userId = " +
            ":userloggedId) OR ((a.isPublic IS true) AND (a.state != 'CREATED' AND a.state != 'DELETED')) OR ((a.id " +
            "IN (SELECT ua.api.id FROM UserApi AS ua WHERE ua.api.id = a.id and ua.user.userId = :userloggedId)) AND " +
            "(a.state != 'CREATED' AND a.state != 'DELETED'))) AND (a.identification LIKE %:apiId% AND a.user.userId " +
            "LIKE %:userId%)) ORDER BY a.identification asc")
    ...
    @Query("SELECT a FROM Api as a WHERE (((:userloggedRole = 'ROLE_ADMINISTRATOR') OR (a.user.userId = " +
            ":userloggedId) OR ((a.isPublic IS true) AND (a.state != 'CREATED' AND a.state != 'DELETED')) OR ((a.id " +
            "IN (SELECT ua.api.id FROM UserApi AS ua WHERE ua.api.id = a.id and ua.user.userId = :userloggedId)) AND " +
            "(a.state != 'CREATED' AND a.state != 'DELETED'))) AND (a.identification LIKE %:apiId% AND a.state = " +
            ":state AND a.user.userId LIKE %:userId%)) ORDER BY a.identification asc")
