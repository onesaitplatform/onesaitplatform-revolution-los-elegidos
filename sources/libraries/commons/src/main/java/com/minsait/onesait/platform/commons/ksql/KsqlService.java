...
public class KsqlService {
    ...
    private void processDeleteQueriesFromConcreteSink(RunningQuery query,
            String sinkIdentification) throws KsqlExecutionException {
        for (final String sink : query.getSinks()) {
            if (sink.equalsIgnoreCase(sinkIdentification)) {
                // Delete Query
                final RestResponse<KsqlEntityList> response = ksqlClient.makeKsqlRequest(
                        "terminate " + query.getId().getId() + ";");
                if (response.get() instanceof KsqlErrorMessage) {
                    final KsqlErrorMessage error = (KsqlErrorMessage) response.get();
