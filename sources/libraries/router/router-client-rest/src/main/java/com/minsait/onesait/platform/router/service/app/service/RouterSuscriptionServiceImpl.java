...
public class RouterSuscriptionServiceImpl
    ...
    public OperationResultModel execute(SuscriptionModel model) {
        ...
        if (operation.equalsIgnoreCase("SUSCRIBE") || operation.equalsIgnoreCase(
                SuscriptionModel.OperationType.SUSCRIBE.name())) {
            quote = restTemplate.postForObject(routerStandaloneURL + "/suscribe", model, OperationResultModel.class);
        } else if (operation.equalsIgnoreCase("UNSUSCRIBE") || operation.equalsIgnoreCase(
                SuscriptionModel.OperationType.UNSUSCRIBE.name())) {
