...
public class RouterServiceImpl implements RouterService, RouterClient<NotificationModel, OperationResultModel> {
    ...
    public OperationResultModel execute(NotificationModel input) {
        try {
            ...
            if (operation.equalsIgnoreCase("POST") || operation.equalsIgnoreCase(
                    OperationModel.OperationType.INSERT.name())) {
                quote = restTemplate.exchange(routerStandaloneURL + "/insert", HttpMethod.POST,
                                              new HttpEntity<>(input, addCorrelationHeader()),
                                              OperationResultModel.class).getBody();
            } else if (operation.equalsIgnoreCase("PUT") || operation.equalsIgnoreCase(
                ...
                quote = restTemplate.exchange(routerStandaloneURL + "/update", HttpMethod.PUT, request,
                                              OperationResultModel.class).getBody();
            } else if (operation.equalsIgnoreCase("DELETE") || operation.equalsIgnoreCase(
                    OperationModel.OperationType.DELETE.name())) {
                final org.springframework.http.HttpEntity<NotificationModel> request = new HttpEntity<>(input,
                                                                                                        addCorrelationHeader());
                quote = restTemplate.exchange(routerStandaloneURL + "/delete", HttpMethod.DELETE, request,
                                              OperationResultModel.class).getBody();
            } else if (operation.equalsIgnoreCase("GET") || operation.equalsIgnoreCase(
