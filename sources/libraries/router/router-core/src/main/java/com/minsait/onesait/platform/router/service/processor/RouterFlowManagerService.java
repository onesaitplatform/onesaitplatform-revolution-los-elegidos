...
public class RouterFlowManagerService {
            ...
            if (e.getCause() != null) {
                ...
                if (e.getCause() instanceof DBPersistenceException) {
                    ...
                    if (!dbex.getErrorsResult().isEmpty())
                        output.setMessage(
                                ERROR_MESSAGE_STR + dbex.getDetailedMessage() + "\", \"errors\" : " + (new Gson()).toJson(
                                        dbex.getErrorsResult()) + " }}");
                    else if (dbex.getCause() != null)
                        output.setMessage(
                                ERROR_MESSAGE_STR + dbex.getDetailedMessage() + "\", \"errors\" : [" + (new Gson()).toJson(
                                        new ErrorResult(dbex.getCause().getMessage())) + "] }}");
                    else
    ...
    public NotificationCompositeModel executeCrudOperations(
            NotificationCompositeModel compositeModel) throws GenericOPException, RouterCrudServiceException {
        ...
        try {
            if (METHOD.equalsIgnoreCase(ApiOperation.Type.GET.name()) || METHOD.equalsIgnoreCase(
                    OperationModel.OperationType.QUERY.name())) {
                final OperationResultModel result = routerCrudService.query(model);
                compositeModel.setOperationResultModel(result);
            } else if (METHOD.equalsIgnoreCase(ApiOperation.Type.POST.name()) || METHOD.equalsIgnoreCase(
                    OperationModel.OperationType.INSERT.name())) {
                final OperationResultModel result = routerCrudService.insert(model);
                compositeModel.setOperationResultModel(result);
            } else if (METHOD.equalsIgnoreCase(ApiOperation.Type.PUT.name()) || METHOD.equalsIgnoreCase(
                    OperationModel.OperationType.UPDATE.name())) {
                final OperationResultModel result = routerCrudService.update(model);
                compositeModel.setOperationResultModel(result);
            } else if (METHOD.equalsIgnoreCase(ApiOperation.Type.DELETE.name()) || METHOD.equalsIgnoreCase(
    ...
    private List<AdviceNotificationModel> getScriptsAndNodereds(NotificationCompositeModel compositeModel) {
        ...
        final Map<String, AdviceNotificationService> map = applicationContext.getBeansOfType(
                AdviceNotificationService.class);
                ...
                try {
                    ...
                    if (model != null) {
                        final OperationModel operationModel = OperationModel.builder(model.getOntologyName(),
                                                                                     ...
                                                                                     model.getUser(),
                                                                                     Source.INTERNAL_ROUTER).body(
                                appendOIDForSQL(model.getQuery(),
                                                compositeModel.getOperationResultModel().getResult())).queryType(
                                QueryType.valueOf(model.getQueryType().name())).build();
    ...
    public void notifyKafkaTopics(NotificationCompositeModel compositeModel) {
        ...
        if (messageType == OperationType.POST || messageType == OperationType.INSERT) {
            ...
            final Map<String, KafkaTopicNotificationService> map = applicationContext.getBeansOfType(
                    KafkaTopicNotificationService.class);
    ...
    public void notifyRulesEngine(NotificationCompositeModel compositeModel) {
        ...
        if ((messageType == OperationType.POST || messageType == OperationType.INSERT) && rulesEngineNotificationService.notifyToEngine(
                ontology) && !Source.RULES_ENGINE.equals(source)) {
