...
public enum Command {
     ...
     */
    GET_HUMIDITY("print(sensehat.get_humidity())"), GET_TEMPERATURE("print(sensehat.get_temperature())"),
     ...
     * LED Matrix
     */
    SET_ROTATION("sensehat.set_rotation(%s)"), SET_PIXEL("sensehat.set_pixel(%s, %s, %s, %s, %s)"),
    SET_PIXELS("sensehat.set_pixels([%s])"), CLEAR("sensehat.clear()"), CLEAR_WITH_COLOR("sensehat.clear(%s, %s, %s)"),
     ...
     */
    GET_EVENTS(
            "e = sensehat.stick.get_events();" + "print(str('|'.join('{}@{}@{}'.format(i.action, i.direction, i" +
                    ".timestamp) for i in e)))"),
    WAIT_FOR_EVENT_EMPTY_BUFFER(
