...
public class KuduTableGenerator {
    ...
    public KuduTable builTable(String ontologyName, String schema, Map<String, String> config) {
        ...
        if (props.length() == 1 && ((JSONObject) props.get(requiredProps.get(0))).has(
                "$ref") && ((JSONObject) props.get(requiredProps.get(0))).has("type")) {
    ...
    public KuduTable build(String name, JSONObject props, List<String> requiredProperties, Map<String, String> cmap) {
        ...
        @SuppressWarnings("unchecked") final Iterator<String> it = props.keys();
            ...
            if (Arrays.stream(primarykey).anyMatch(key::equals)) {
                if (isPrimitive(o)) {
                    table.getColumns().add(
                            new HiveColumn(key, pickPrimitiveType(key, o), requiredProperties.contains(key)));
                } else {
    ...
    public boolean isPrimitive(JSONObject o) {
        ...
        return JsonFieldType.getPRIMITIVE_TYPES().contains(jsonType) && !((o.has(FORMAT) && "date-time".equals(
                o.get(FORMAT))) && "string".equals(jsonType));
    }
    ...
    public boolean isGeometry(JSONObject o) {
        ...
        try {
            ...
            if ((JsonFieldType.OBJECT_FIELD).equalsIgnoreCase(jsonType) && o.has(
                    JsonFieldType.PROPERTIES_FIELD) && o.getJSONObject(JsonFieldType.PROPERTIES_FIELD).has(
                    JsonFieldType.TYPE_FIELD) && o.getJSONObject(JsonFieldType.PROPERTIES_FIELD).getJSONObject(
                    JsonFieldType.TYPE_FIELD).has("enum")) {
                final JSONArray enume = o.getJSONObject(JsonFieldType.PROPERTIES_FIELD).getJSONObject(
                        JsonFieldType.TYPE_FIELD).getJSONArray("enum");
    ...
    public boolean isTimestamp(JSONObject o) {
        ...
        try {
            ...
            if ((JsonFieldType.OBJECT_FIELD).equalsIgnoreCase(jsonType) && o.get(
                    JsonFieldType.PROPERTIES_FIELD) != null) {
                final JSONObject other = (JSONObject) o.get(JsonFieldType.PROPERTIES_FIELD);
                if (other.get("$date") != null) {
                    result = true;
                }
            } else if (o.has(FORMAT) && "date-time".equals(o.get(FORMAT)) && JsonFieldType.STRING_FIELD.equals(
                    jsonType)) {
                return true;
            }
        } catch (final Exception e) {
