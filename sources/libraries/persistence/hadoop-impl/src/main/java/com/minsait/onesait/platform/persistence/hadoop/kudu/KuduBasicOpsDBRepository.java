...
public class KuduBasicOpsDBRepository implements BasicOpsDBRepository {
    ...
    public ComplexWriteResult insertBulk(String ontology, String schema, List<String> instances, boolean order,
        ...
        if (instances != null) {
            ...
            if (!errors.isEmpty()) {
                List<ErrorResult> eResult = errors.stream().map(dbpex -> dbpex.getErrorsResult().get(0)).collect(
                        Collectors.toList());
    ...
    private void translateUpdateToRow(String query, PartialRow prow, Schema schema) throws JSQLParserException {
        final net.sf.jsqlparser.statement.update.Update statement =
                (net.sf.jsqlparser.statement.update.Update) parserManager.parse(
            ...
            switch (schema.getColumn(key).getType()) {
                ...
                case UNIXTIME_MICROS:
                    prow.addTimestamp(key, new Timestamp(ISODateTimeFormat.dateTimeParser().parseDateTime(
                            o.substring(1, o.length() - 1)).getMillis()));
                    break;
                case BOOL:
            ...
            if (fieldVal.length != 2) {
                ...
                switch (schema.getColumn(key).getType()) {
                    case STRING:
                        ...
                        break;
                    case UNIXTIME_MICROS:
                        prow.addTimestamp(key, Timestamp.valueOf(
                                o.substring(1, o.length() - 1).replace("T", " ").replace("Z", "")));
                        break;
                    case BOOL:
                        prow.addBoolean(key, Boolean.getBoolean(o));
                        break;
