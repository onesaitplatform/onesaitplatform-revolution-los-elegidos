    ...
    public void createIndex(String sentence) {
        ...
        try {
            ...
            try {
                ...
                log.error(
                        "Query bad formed:" + pquery + ".Expected db.<collection>.createIndex({<attribute>:1}," +
                                "{name:'name_index',....})");
                throw new DBPersistenceException(
                        "Query bad formed:" + pquery + ".Expected db.<collection>.createIndex({<attribute>:1}," +
                                "{name:'name_index',....})");
        ...
        if (mongoDbConnector.count(database, ontology, "count(" + query + ")") > 0) {
            ...
            if (mongoDbConnector.getCredentials().isEnableMongoDbAuthentication()) {
                pb = new ProcessBuilder("mongoexport", "--host",
                                        mongoDbConnector.getConnection().getServerAddressList().get(0).getHost(),
                                        "--db", database, "--username", mongoDbConnector.getCredentials().getUsername(),
                                        "--password", mongoDbConnector.getCredentials().getPassword(), "--collection",
                                        ontology, "--query", query, "--out", path, "--authenticationDatabase",
