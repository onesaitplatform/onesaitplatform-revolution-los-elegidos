...
public class QueryAsTextMongoDBImpl implements QueryAsTextDBRepository {
    ...
    private void checkQueryIs4Ontology(String ontology, String query, boolean sql) throws GenericOPException {
        ...
        if (sql) {
            if (query.toLowerCase().indexOf("from ") == -1 && query.toLowerCase().indexOf(
                    "update ") == -1 && query.toLowerCase().indexOf("join ") == -1)
                throw new QueryNativeFormatException("Malformed SQL Query");
            else if (query.toLowerCase().indexOf("from " + ontology.toLowerCase()) == -1 && query.toLowerCase().indexOf(
                    "update " + ontology.toLowerCase()) == -1 && query.toLowerCase().indexOf(
                    "join " + ontology.toLowerCase()) == -1)
