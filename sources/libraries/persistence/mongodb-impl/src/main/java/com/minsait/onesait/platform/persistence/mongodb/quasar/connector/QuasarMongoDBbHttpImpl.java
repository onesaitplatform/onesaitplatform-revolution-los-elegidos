...
public class QuasarMongoDBbHttpImpl implements QuasarMongoDBbHttpConnector {
    ...
    private boolean compileQueries() {
        try {
            return ((Boolean) resourcesService.getGlobalConfiguration().getEnv().getDatabase().get(
                    "mongodb-quasar-compile")).booleanValue();
    ...
    private int getMaxRegisters() {
        return ((Integer) resourcesService.getGlobalConfiguration().getEnv().getDatabase().get(
        ...
        try {
            ...
            if (!ontologyDataService.refJsonSchema(schema).equals("")) {
                ...
                if (parentNode != null && parentNode.trim().length() > 0) {
                    ...
                    while (matcher.find()) {
                        ...
                        if (predicateQuery.startsWith(collection)) {
                            ...
                            if (!m3.matches()) {
                                query = query.substring(0, foundReplace.length()) + " from " + predicateQuery.replace(
                ...
                throw new DBPersistenceException(
                        "Query for ontology " + collection + " contains *, please indicate explicitly the fields you " +
                                "want to query");
            }
            return query;
