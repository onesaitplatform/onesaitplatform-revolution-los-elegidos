...
public class MongoBasicOpsDBRepository implements BasicOpsDBRepository {
    ...
    private static final List<String> METRICS_ONTOLOGIES = Arrays.asList(
            new String[]{"MetricsOntology", "MetricsOperation", "MetricsApi", "MetricsControlPanel",
    ...
    private int getMaxRegisters() {
        return ((Integer) resourcesServices.getGlobalConfiguration().getEnv().getDatabase().get(
                "queries-limit")).intValue();
    }
    ...
    public MultiDocumentOperationResult updateNativeByObjectIdAndBodyData(String collection, String objectId,
            String bodyData) {
        final String updateQuery = "db." + collection + ".update({\"_id\": {\"$oid\" : \"" + objectId + "\" }}, " +
                "{$set:" + bodyData + " })";
    ...
    private String processMetricsQuery(String ontology, String query) throws DBPersistenceException {
        try {
            ...
            List<String> cerosInInterval = this.queryNative(METRICS_BASE, queryData.get(
                    MetricQueryResolver.STATEMENT_CEROS_COMPLETION));
