...
public class ExternalApiRestOpsDBRepository implements BasicOpsDBRepository {
    ...
    public String insert(String ontology, String schema, String instance) {
        ...
        List<OntologyRestOperation> operations = this.ontologyRestOperationService.getAllOperationsFromOntologyRest(
                ontologyRest);
        Optional<OntologyRestOperation> insertOpertaion = operations.stream().filter(
                operation -> operation.getDefaultOperationType() == DefaultOperationType.INSERT).findFirst();
        if (insertOpertaion.isPresent()) {
            String completeUrl = ontologyRest.getBaseUrl() + insertOpertaion.get().getPath();
            APIRestResponse response = this.apiRestClient.invokePost(completeUrl, instance, Optional.of(
                    this.getHeadersAndSecurity(ontologyRest)), Optional.of(new HashMap<String, String>()),
            ...
            if (response.getResponse() == 200) {
                ...
                throw new DBPersistenceException(new ErrorResult(ErrorResult.PersistenceType.API, response.getBody()),
                                                 "Unable to insert data in REST API Datasource. Error " + response.getResponse() + "." + response.getBody());
            }
        } else {
    ...
    private MultiDocumentOperationResult update(String ontology, String updateStmt, Optional<String> data) {
        ...
        if (parserResult.getOperation() != null) {
            ...
            List<OntologyRestOperation> operations = this.ontologyRestOperationService.getAllOperationsFromOntologyRest(
                    ontologyRest);
            updateOperation = operations.stream().filter(
                    operation -> operation.getDefaultOperationType() == DefaultOperationType.UPDATE_BY_ID).findFirst().orElse(
                    null);
        }
        // if update operation found
        if (updateOperation != null) {
            List<OntologyRestOperationParam> params = ontologyRestOperationService.getOntologyRestOperationParams(
                    updateOperation);
            ...
            APIRestResponse response = this.apiRestClient.invokePut(completeUrl, dataToUpdate, Optional.of(
                    this.getHeadersAndSecurity(ontologyRest)), Optional.of(parserResult.getPathParams()),
            ...
            if (response.getResponse() == 200) {
                ...
                throw new DBPersistenceException(new ErrorResult(ErrorResult.PersistenceType.API, response.getBody()),
                                                 "Unable to update data in REST API Datasource. Error " + response.getResponse() + "." + response.getBody());
            }
    ...
    public String querySQLAsJson(String ontology, String query) {
        ...
        if (analyzedQuery.getQueryType() != QueryType.SELECT) {
            throw new DBPersistenceException(new NotSupportedOperationException(
                    "Only SELECT operations are allowed but " + analyzedQuery.getQueryType().toString() + " was " +
                            "received."));
        }
        ...
        if (operationName == null) {
            ...
            // getAll defined
            List<OntologyRestOperation> operations = ontologyRestOperationService.getAllOperationsFromOntologyRest(
                    ontologyRest);
            Optional<OntologyRestOperation> optionalOperation = operations.stream().filter(
                    oper -> oper.getDefaultOperationType() == DefaultOperationType.GET_ALL).findAny();
        ...
        if (response.getResponse() == 200) {
            ...
            List<String> selectedFfields = analyzedQuery.getProjectionFields().stream().filter(
                    field -> !("_id".equalsIgnoreCase(field) || "*".equalsIgnoreCase(field))).collect(
                    Collectors.toList());
            ...
            throw new DBPersistenceException(new ErrorResult(ErrorResult.PersistenceType.API, response.getBody()),
                                             "Unable to run query in REST API Datasource. Error " + response.getResponse() + "." + response.getBody());
        }
    }
    ...
    public MultiDocumentOperationResult delete(String ontology, boolean includeIds) {
        ...
        List<OntologyRestOperation> operations = ontologyRestOperationService.getAllOperationsFromOntologyRest(
                ontologyRest);
        OntologyRestOperation deleteAllOperation = operations.stream().filter(
                operation -> operation.getDefaultOperationType() == DefaultOperationType.DELETE_ALL).findFirst().orElse(
                null);
        ...
        if (deleteAllOperation != null) {
            ...
            if (response.getResponse() == 200) {
                ...
                throw new DBPersistenceException(new ErrorResult(ErrorResult.PersistenceType.API, response.getBody()),
                                                 "Unable to delete data in REST API Datasource. Error " + response.getResponse() + "." + response.getBody());
            }
        } else {
    ...
    public MultiDocumentOperationResult deleteNative(String collection, String query, boolean includeIds) {
        ...
        if (analyzedQuery.getOperation() != null) {
            ...
            List<OntologyRestOperation> operations = this.ontologyRestOperationService.getAllOperationsFromOntologyRest(
                    ontologyRest);
            deleteOperation = operations.stream().filter(
                    operation -> operation.getDefaultOperationType() == DefaultOperationType.DELETE_BY_ID).findFirst().orElse(
                    null);
        }
        ...
        if (deleteOperation != null) {
            ...
            APIRestResponse response = this.apiRestClient.invokeDelete(completeUrl, Optional.of(headers),
                                                                       Optional.of(analyzedQuery.getPathParams()),
                                                                       Optional.of(analyzedQuery.getQueryParams()));
            if (response.getResponse() == 200) {
