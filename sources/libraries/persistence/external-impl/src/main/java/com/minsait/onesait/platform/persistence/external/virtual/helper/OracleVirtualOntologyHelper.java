            ...
            if (!query.toUpperCase().contains(ROWNUM_STR) && !query.toUpperCase().contains("COUNT(*)")) {
                ...
                ExpressionDeParser expressionDeparser = new ExpressionDeParser() {
                    ...
                    private void validateRowNum(Expression expr) {
                        if ((expr instanceof MinorThan || expr instanceof EqualsTo) && ((ComparisonOperator) expr).getLeftExpression().toString().equals(
                                ROWNUM_STR)) {
                            int currentLimit = Integer.parseInt(
                                    (((ComparisonOperator) expr).getRightExpression()).toString());
