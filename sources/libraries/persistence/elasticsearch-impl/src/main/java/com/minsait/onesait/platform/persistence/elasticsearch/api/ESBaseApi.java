...
public class ESBaseApi {
    ...
    public static final String QUERY_ALL_SIZE =
            START_QUERY + SIZE_JSON + "  ,\"from\": 0\r\n" + "  ,\"query\":\r\n" + "   {\r\n" + "    \"match_all\": " +
                    "{}\r\n" + "   }\r\n" + "}";
    ...
    public String updateIndex(String index, String type, String id, String jsonData) {
        try {
            DocumentResult result = getHttpClient().execute(
                    new Update.Builder(jsonData).index(index).type(type).id(id).build());
            return result.getJsonString();
        } catch (IOException e) {
