...
public class ESDeleteService {
    ...
    public boolean deleteById(String index, String type, String id) {
        try {
            DocumentResult d = connector.getHttpClient().execute(
                    new Delete.Builder(id).index(index).type(type).build());
    ...
    public MultiDocumentOperationResult deleteByQuery(String index, String type, String jsonQueryString,
        ...
        try {
            ...
            if (resultOp.isSucceeded()) {
                result.setCount(resultOp.getJsonObject().getAsJsonObject("_indices").getAsJsonObject(index).get(
                        "deleted").getAsLong());
            } else {
                result.setCount(-1);
            }
