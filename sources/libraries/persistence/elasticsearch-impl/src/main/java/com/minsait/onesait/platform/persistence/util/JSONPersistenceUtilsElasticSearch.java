...
public class JSONPersistenceUtilsElasticSearch {
    ...
    public static String getElasticSearchSchemaFromJSONSchema(final String schemaString) {
        try {
            final JsonObject jsonSchema = new JsonParser().parse(schemaString).getAsJsonObject();
            if (jsonSchema.has(PROPERTIES) && jsonSchema.get(PROPERTIES).isJsonObject() && !jsonSchema.getAsJsonObject(
    ...
    public static JsonObject getElasticIndexFromObject(final JsonObject jsonSchema, final JsonObject jsonObject,
        ...
        if (property.has("type") && !property.get("type").isJsonNull() && property.get(
                "type").isJsonPrimitive() && property.get("type").getAsJsonPrimitive().isString()) {
            ...
            switch (propType) {
                case "array":
                    if (property.has(ITEMS)) {
                        ...
                        if (property.get(ITEMS).isJsonArray() && property.getAsJsonArray(ITEMS).size() > 0) {
                            ...
                            final List<JsonObject> itemsObjects = StreamSupport.stream(
                                    property.get(ITEMS).getAsJsonArray().spliterator(), false).map(it -> {
                                if (it.isJsonObject()) {
                                    return it.getAsJsonObject().has("$ref") ? resolveSchemaRef(jsonSchema,
                                                                                               it.getAsJsonObject())
                                            : it.getAsJsonObject();
                                } else {
                                    throw new JsonSyntaxException(
                                            PROPERTY + propName + " is type array, but an items's field is not an " +
                                                    "object.");
                                }
                            }).collect(Collectors.toList());

                            final int itemsSize = itemsObjects.stream().map(itemJsonArray -> {
                                if (itemJsonArray.has("type") && itemJsonArray.get(
                                        "type").isJsonPrimitive() && itemJsonArray.get(
                            ...
                            if (itemsSize > 1)
                                throw new JsonSyntaxException(
                                        PROPERTY + propName + " is type array and has differents types in it. Elastic" +
                                                " search 6+ does not support multitype mapping.");
                ...
                case "object":
                    if (property.has(PROPERTIES) && property.get(
                            PROPERTIES).isJsonObject() && !property.getAsJsonObject(PROPERTIES).entrySet().isEmpty()) {
                        ...
                        } else if (objectProperties.entrySet().size() == 2 && objectProperties.has(
                                "coordinates") && objectProperties.has("type") && objectProperties.get(
                                "coordinates").isJsonObject() && objectProperties.get(
                                "type").isJsonObject() && objectProperties.getAsJsonObject("type").has(
                                "enum") && objectProperties.getAsJsonObject("type").get(
                                "enum").isJsonArray() && objectProperties.getAsJsonObject("type").get(
                                "enum").getAsJsonArray().size() == 1 && objectProperties.getAsJsonObject("type").get(
                                "enum").getAsJsonArray().get(0).isJsonPrimitive() && objectProperties.getAsJsonObject(
                                "type").get("enum").getAsJsonArray().get(
                                0).getAsJsonPrimitive().isString() && geoShapes.contains(
                                objectProperties.getAsJsonObject("type").get("enum").getAsJsonArray().get(
                                        0).getAsString().toLowerCase())) {
                            ...
                            objectProperties.entrySet().stream().forEach(pro -> {
                                if (pro.getValue().isJsonObject())
                                    elasticProperty.add(pro.getKey(), getElasticIndexFromObject(jsonSchema,
                                                                                                pro.getValue().getAsJsonObject(),
                                                                                                pro.getKey()));
                                else
                                    throw new JsonSyntaxException(
                                            PROPERTY + pro.getKey() + " of property " + propName + " is not an json " +
                                                    "object.");
                            });
                ...
                case "string":
                    // If is a date format...
                    if (property.has(FORMAT) && property.get(FORMAT).isJsonPrimitive() && property.getAsJsonPrimitive(
                            FORMAT).isString() && (property.getAsJsonPrimitive(FORMAT).getAsString().equalsIgnoreCase(
                            "date-time") || property.getAsJsonPrimitive(FORMAT).getAsString().equalsIgnoreCase(
                            "date"))) {
