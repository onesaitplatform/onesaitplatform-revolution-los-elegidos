...
public class ElasticSearchBySQLPluginIntegrationTest {
    ...
    public void setUp() throws Exception {
        if (!init) {
            ...
            final List<String> list = ESInsertService.readLines(
                    new File(this.getClass().getClassLoader().getResource("accounts.json").toURI()));

            final List<String> result = list.stream().filter(x -> x.startsWith("{\"account_number\"")).collect(
                    Collectors.toList());
    ...
    public void test_SQL_SelectGroup() {
        try {
            ...
            // group by gender";
            final String query =
                    "select topHits('size'=3,'include'='age,firstname',age='desc') from " + TEST_INDEX_ACCOUNT + " " +
                            "group by gender";
            final String result = httpConnector.queryAsJson(query, 500);
            log.info("Returned:" + result);
    ...
    public void test_SQL_CountAndGroup() {
        try {
            final String query = "select count(*),sum(balance) from " + TEST_INDEX_ACCOUNT + " where age in (35,36) " +
                    "group by gender";
            final String result = httpConnector.queryAsJson(query, 1);
            log.info("Returned:" + result);
    ...
    public void test_SQL_Select_WithFilter() {
        try {
            final String query = "select age , firstname from " + TEST_INDEX_ACCOUNT + " where age > 31 order by age " +
                    "desc";
            final String result = httpConnector.queryAsJson(query, 1000);
            log.info("Returned:" + result);
