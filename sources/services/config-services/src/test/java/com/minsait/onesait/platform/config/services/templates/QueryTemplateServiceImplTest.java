...
public class QueryTemplateServiceImplTest {
    ...
    public void given_AQueryTemplateAndASetOfVariables_When_TheVariablesAreReplaced_Then_AValidQueryTemplateIsGenerated() {
        ...
        String query = "var pipeline = [];\n" + "\n" + "pipeline.push(\n" + "    {\n" + "        $match: {\n" + "    " +
                "      \"Test1.temperature.timestamp\": {\"$gte\" : ISODate(\"@var_from\"), \"$lt\" : ISODate" +
                "(\"@var_to\") }\n" + "        }\n" + "    }\n" + ");\n" + "db.getCollection(\"AssetTestArray2\")" +
                ".aggregate(pipeline);\n" + "return \"db.AssetTestArray2.aggregate(\" + JSON.stringify(pipeline) + " +
                "\")\";\n" + "";

        String newQuery = "var pipeline = [];\n" + "\n" + "pipeline.push(\n" + "    {\n" + "        $match: {\n" + " " +
                "         \"Test1.temperature.timestamp\": {\"$gte\" : ISODate(\"2017-01-01T00:00:00.000Z\"), \"$lt\"" +
                " : ISODate(\"2017-01-02T00:00:00.000Z\") }\n" + "        }\n" + "    }\n" + ");\n" + "db" +
                ".getCollection(\"AssetTestArray2\").aggregate(pipeline);\n" + "return \"db.AssetTestArray2.aggregate" +
    ...
    @Test
    public void given_AQueryTemplate_When_TheQueryTemplateIsProcessed_Then_TheJavascriptCodeIsInterpreted() throws NoSuchMethodException, ScriptException {
