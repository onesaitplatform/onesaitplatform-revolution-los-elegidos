...
public class ConfigurationTest {
    ...
    public void parseNginxConfFile_andRemoveControlpanelFromConf() throws IOException {
        ...
        // });
        locations.forEach(l -> {
            final List<String> filtered = services.stream().filter(
                    s -> l.toString().startsWith("location /".concat(myMap.get(s) == null ? s : myMap.get(s)))).collect(
                    Collectors.toList());
        ...
        org.junit.Assert.assertTrue(
                conf.findAll(NgxConfig.BLOCK, "http", "server", "location /controlpanel ").isEmpty());
    }
