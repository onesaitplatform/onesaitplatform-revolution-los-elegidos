...
public class KsqlResourceServiceImpl implements KsqlResourceService, KafkaTopicNotificationService {
    ...
    private final Pattern creation = Pattern.compile(
            "(?i)\\s*CREATE\\s+(TABLE|STREAM)\\s+(\\w+)\\s+(\\(((?!\\)\\s+WITH\\s+\\().)+\\))\\s*(WITH\\s+\\(" +
                    "(\\w+\\s*\\=\\s*'[\\w]+')?(\\s*,\\s*(\\w+\\s*\\=\\s*'[\\w]+'))*\\))?.*;");
    private final Pattern creationAs = Pattern.compile(
            "(?i)\\s*CREATE\\s+(TABLE|STREAM)\\s+(\\w+)\\s+(\\s*WITH\\s*\\((\\w+\\s*\\=\\s*'[\\w]+')?(\\s*,\\s*" +
                    "(\\w+\\s*\\=\\s*'[\\w]+'))*\\))?\\s*AS\\s+.*FROM\\s+((\\w+)(\\s+(LEFT\\s+JOIN\\s+)(\\w+))?).*;");
    Pattern insertInto = Pattern.compile(
            "(?i)\\s*(INSERT)\\s+INTO\\s+(\\w+)\\s+SELECT\\s+((?!\\s+FROM\\s+).)*\\s+FROM\\s+((\\w+)(\\s+" +
                    "(LEFT\\s+JOIN\\s+)(\\w+))?).*;");
    ...
    public void validateKsqlResource(KsqlResource newKsqlResource) {
        ...
        if (ksqlResourceRepository.findByIdentificationAndIdNot(newKsqlResource.getIdentification(), id) != null) {
            ...
            throw new KsqlResourceServiceException(
                    "The KsqlResource Identification is already being used. Identification = " + newKsqlResource.getIdentification());
        }
    ...
    public void updateKsqlResource(KsqlResourceForUpdate ksqlresourceForUpdate) throws KsqlExecutionException {
        ...
        final KsqlResource preModifKsqlResource = ksqlResourceRepository.findByIdentification(
        ...
        if (ksqlresourceForUpdate.isStatemenChanged()) {
            ...
            try {
                ...
                ksqlService.executeKsqlRequest(
                        "drop " + ksqlresourceForUpdate.getCurrentKsqlResourceType() + " " + ksqlresourceForUpdate.getCurrentIdentification() + ";");
                ...
                log.error("Error creating KSQL {}={} in Server.", ksqlResource.getKsqlType(),
                          ksqlResource.getIdentification());
                throw new KsqlExecutionException(
                        "KsqlResource cannot be created in KSQL server. Identification = " + ksqlResource.getIdentification());
            }
    ...
    private List<String> configureMatchingKsqlResource(KsqlResource ksqlResource, Matcher matcher,
        ...
        if (hasWithClause && withClause != null) {
            final String[] listOfPorperties = withClause.trim().replaceAll("(?i)^WITH\\s*\\(", "").replaceAll(
                    "\\s*\\).*", "").split(",");
