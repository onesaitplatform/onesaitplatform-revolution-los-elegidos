...
public class ApiManagerServiceImpl implements ApiManagerService {
    ...
    public String createApi(Api api, String operationsObject, String authenticationObject) {
        ...
        try {
            final String numversionData =
                    "{\"identification\":\"" + api.getIdentification() + "\",\"apiType\":\"" + api.getApiType() + "\"}";
    ...
    public UserApi updateAuthorization(String apiId, String userId) {
        ...
        if (userApi == null) {
            ...
            return apiRepository.save(api).getUserApiAccesses().stream().filter(
                    ua -> ua.getUser().equals(user) && ua.getApi().equals(api)).findFirst().orElse(userApi);
    ...
    public boolean hasUserAccess(String apiId, String userId) {
        ...
        if (user.equals(api.getUser()) || user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.name()))
            return true;
        if ((api.isPublic() || userApiRepository.findByApiIdAndUser(apiId,
                                                                    userId) != null) && (!api.getState().toString().equals(
                "CREATED") || !api.getState().toString().equals("DELETED")))
            return true;
    ...
    private boolean evaluatePublished(ApiStates oldState) {
        return (oldState.equals(ApiStates.CREATED) || oldState.equals(ApiStates.DEVELOPMENT) || oldState.equals(
                ApiStates.PUBLISHED));
    }
