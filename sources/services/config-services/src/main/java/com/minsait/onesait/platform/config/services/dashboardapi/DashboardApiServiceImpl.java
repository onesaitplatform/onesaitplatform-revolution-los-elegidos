...
public class DashboardApiServiceImpl implements DashboardApiService {
    ...
    private static final String[] COLORS = {"rgba(40,146,215, 0.8)", "rgba(119,178,131, 0.8)", "rgba(178,131,119, 0" +
            ".8)", "rgba(178,161,119, 0.8)", "rgba(247,179,121, 0.8)", "rgba(139,165,160, 0.8)", "rgba(254, 246, 240," +
            " 0.8)", "rgba(207, 206, 229, 0.8)"};

    private static final String CREATE_GADGET_RESPONSE = "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", " +
            "\"message\":\"gadget not created correctly\",\"data\":{}}";
    private static final String CANNOT_CREATE_GADGET = "Cannot create gadget: Information MapConf data is necessary ";
    private static final String GADGET_NOT_UPDATED_CORRECTLY = "{\"requestcode\":\"updateGadget\"," +
            "\"status\":\"ERROR\", \"message\":\"gadget not updated correctly\",\"data\":{}}";
    ...
    public String createGadget(String json, String userId) {
        ...
        try {
            ...
            if ((commandDTO.getInformation().getOntology() == null || commandDTO.getInformation().getOntology().trim().length() == 0) && (commandDTO.getInformation().getDataSource() == null || commandDTO.getInformation().getDataSource().trim().length() == 0)) {
                log.error("Cannot create gadget: ontology or datasource is necessary");
                return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                        "correctly ontology or datasource is necessary\",\"data\":{}}";
            }
            if (commandDTO.getInformation().getDataSource() != null && commandDTO.getInformation().getDataSource().trim().length() > 0 && commandDTO.getInformation().getGadgetType().equals(
                    MAP)) {
                final GadgetDatasource datasource = gadgetDatasourceService.getDatasourceByIdentification(
                        commandDTO.getInformation().getDataSource());
                if (datasource == null) {
                    log.error("Cannot create gadget: valid datasource is necessary");
                    return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                            "correctly valid datasource is necessary\",\"data\":{}}";
                }
            }
            if (commandDTO.getInformation().getDashboard() == null || commandDTO.getInformation().getDashboard().trim().length() == 0) {
                log.error("Cannot create gadget: dashboard is necessary");
                return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                        "correctly dashboard is necessary\",\"data\":{}}";
            }
            if (commandDTO.getInformation().getGadgetName() == null) {
                log.error("Cannot create gadget: gadgetName is necessary");
                return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                        "correctly gadgetName is necessary\",\"data\":{}}";
            }
            ...
            if (commandDTO.getInformation().getGadgetType() == null || commandDTO.getInformation().getGadgetType().trim().length() == 0) {
                log.error("Cannot create gadget: gadgetType is necessary");
                return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                        "correctly gadgetType is necessary\",\"data\":{}}";
            }
            if (commandDTO.getInformation().getGadgetType().equals(
                    PIE) || commandDTO.getInformation().getGadgetType().equals(
                    LINE) || commandDTO.getInformation().getGadgetType().equals(
                    RADAR) || commandDTO.getInformation().getGadgetType().equals(
                    MIXED) || commandDTO.getInformation().getGadgetType().equals(BAR)) {
                ...
                if (commandDTO.getInformation().getAxes() == null) {
                    ...
                    return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                            "correctly At least one measure is necessary X and Y\",\"data\":{}}";
                }
                if (CollectionUtils.isEmpty(
                        commandDTO.getInformation().getAxes().getMeasuresX()) || CollectionUtils.isEmpty(
                        commandDTO.getInformation().getAxes().getMeasuresY())) {
                    log.error("Cannot create gadget: At least one measure is necessary X and Y");
                    return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                            "correctly At least one measure is necessary X and Y\",\"data\":{}}";
                }
            } else if (commandDTO.getInformation().getGadgetType().equals(MAP)) {
                if (commandDTO.getInformation().getMapConf() == null || commandDTO.getInformation().getMapConf().getIdentifier() == null || commandDTO.getInformation().getMapConf().getLatitude() == null || commandDTO.getInformation().getMapConf().getLongitude() == null || commandDTO.getInformation().getMapConf().getName() == null) {
                    log.error(CANNOT_CREATE_GADGET);
                    return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                            "correctly Information MapConf data is necessary\",\"data\":{}}";
                }
                if (commandDTO.getInformation().getOntology() == null || commandDTO.getInformation().getOntology().trim().length() == 0) {
                    log.error(CANNOT_CREATE_GADGET);
                    return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                            "correctly Information MapConf data is necessary\",\"data\":{}}";
                }
            } else if ((commandDTO.getInformation().getGadgetType().equals(
                    TABLE) || commandDTO.getInformation().getGadgetType().equals(WORDCLOUD)) && CollectionUtils.isEmpty(
                    commandDTO.getInformation().getColumns())) {

                log.error("Cannot create gadget: Information Columns data is necessary ");
                return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                        "correctly Information Columns data is necessary\",\"data\":{}}";

            }

            if (commandDTO.getInformation().getOntology() != null && (commandDTO.getInformation().getRefresh() == null || commandDTO.getInformation().getRefresh().trim().length() == 0)) {
                log.error("Cannot create gadget: refresh is necessary");
                return "{\"requestcode\":\"newGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                        "correctly refresh is necessary\",\"data\":{}}";
            }
            Gadget gadget = null;

            final GadgetTemplate gadgetTemplate = gadgetTemplateService.getGadgetTemplateByIdentification(
                    commandDTO.getInformation().getGadgetType(), userId);

            if (gadgetTemplate != null) {
                // Create gadget from template
                GadgetDatasource datasource;
                // Use exist datasource
                if (commandDTO.getInformation().getDataSource() != null && commandDTO.getInformation().getDataSource().trim().length() > 0) {
                    datasource = gadgetDatasourceService.getDatasourceByIdentification(
                            commandDTO.getInformation().getDataSource());
                } else {
                ...
                if (commandDTO.getInformation().getGadgetType().equals(MAP)) {
                    ...
                    return mapper.writeValueAsString(responseDTO);
                        ...
                        PIE) || commandDTO.getInformation().getGadgetType().equals(
                        LINE) || commandDTO.getInformation().getGadgetType().equals(
                        BAR) || commandDTO.getInformation().getGadgetType().equals(
                        MIXED) || commandDTO.getInformation().getGadgetType().equals(
                        RADAR) || commandDTO.getInformation().getGadgetType().equals(
                        TABLE) || commandDTO.getInformation().getGadgetType().equals(WORDCLOUD)) {
    ...
    public String updateGadget(String json, String userId) {
        ...
        try {
            ...
            if (gadgetTemplate != null) {
                ...
                for (final Iterator<MeasureDTO> iterator =
                     commandDTO.getInformation().getAxes().getMeasuresY().iterator(); iterator.hasNext(); ) {
                    final MeasureDTO measureDTOY = iterator.next();
                    listValues.add(measureDTOY.getPath());
                }
                return "{\"requestcode\":\"updateGadget\",\"status\":\"Template\", \"filters\":" + mapper.writeValueAsString(
                        commandDTO.getInformation().getFilters()) + ",\"merge\":" + commandDTO.getInformation().isMerge() + " , \"message\":\"properly created gadget\",\"id\":\"" + commandDTO.getInformation().getGadgetId() + TYPE_CONCAT + commandDTO.getInformation().getGadgetType() + "\"}";
                ...
                if (gadget == null)
                    return "{\"requestcode\":\"updateGadget\",\"status\":\"OK\", \"message\":\"properly created " +
                            "gadget\",\"id\":\"" + "" + TYPE_CONCAT + "" + "\"}";

                return "{\"requestcode\":\"updateGadget\",\"status\":\"OK\", \"message\":\"properly created gadget\"," +
                        "\"id\":\"" + gadget.getId() + TYPE_CONCAT + gadget.getType() + "\"}";
            }

        } catch (final IOException e1) {
            log.error("Cannot update gadget", e1);
            return "{\"requestcode\":\"updateGadget\",\"status\":\"ERROR\", \"message\":\"gadget not created " +
                    "correctly\",\"data\":{}}";
        } catch (final GadgetDatasourceServiceException e) {
    ...
    private Gadget updateMap(CommandDTO commandDTO, String userId) {
        ...
        if (!CollectionUtils.isEmpty(listMeasures)) {
            ...
            final String query = "select c." + commandDTO.getInformation().getMapConf().getIdentifier() + " as " +
                    "identifier,c." + commandDTO.getInformation().getMapConf().getName() + " as name, c." + commandDTO.getInformation().getMapConf().getLatitude() + " as latitude , c." + commandDTO.getInformation().getMapConf().getLongitude() + " as longitude from " + ontologyIdentification + " as c";
    ...
    private Gadget createGadgetAndMeasures(CommandDTO commandDTO, String userId) {
        ...
        if (gadgetType.equals(PIE) || gadgetType.equals(LINE) || gadgetType.equals(BAR) || gadgetType.equals(
                MIXED) || gadgetType.equals(RADAR)) {
    ...
    private List<GadgetMeasure> createGadgetAxes(CommandDTO commandDTO, String gadgetType, User user, Gadget gadget,
        ...
        if (gadgetType != null && gadgetType.equals(PIE)) {
            configGadget = "{\"legend\":{\"display\":true,\"fullWidth\":false,\"position\":\"left\"," +
                    "\"labels\":{\"padding\":10,\"fontSize\":11,\"usePointStyle\":false,\"boxWidth\":1}}," +
                    "\"elements\":{\"arc\":{\"borderWidth\":1,\"borderColor\":\"#fff\"}}," +
                    "\"maintainAspectRatio\":false,\"responsive\":true,\"responsiveAnimationDuration\":500," +
                    "\"circumference\":\"6.283185307179586\",\"rotation\":\"6.283185307179586\"," +
                    "\"charType\":\"doughnut\"}";
        } else if (gadgetType != null && gadgetType.equals(LINE)) {
            configGadget = "{\"legend\":{\"display\":true,\"fullWidth\":false,\"position\":\"top\"," +
                    "\"labels\":{\"padding\":10,\"fontSize\":11,\"usePointStyle\":false,\"boxWidth\":2}}," +
                    "\"scales\":{\"yAxes\":[{\"id\":\"#0\",\"display\":true,\"type\":\"linear\"," +
                    "\"position\":\"left\",\"scaleLabel\":{\"labelString\":\"\",\"display\":true," +
                    ...
                    "\"ticks\":{\"fontFamily\":\"Soho\"},\"scaleLabel\":{\"display\":true,\"labelString\":\"\"," +
                    "\"fontFamily\":\"Soho\",\"padding\":4},\"hideLabel\":\"2\",\"gridLines\":{\"display\":true," +
                    "\"borderDash\":[2,4],\"color\":\"#CCC\",\"zeroLineBorderDash\":[2,4]," +
            ...
            configGadget = "{\"legend\":{\"display\":false,\"fullWidth\":false,\"labels\":{\"padding\":10," +
                    "\"fontSize\":11,\"usePointStyle\":false,\"boxWidth\":2}},\"scales\":{\"yAxes\":[{\"id\":\"#0\"," +
                    "\"display\":true,\"type\":\"linear\",\"position\":\"left\",\"scaleLabel\":{\"labelString\":\"\"," +
                    "\"display\":true,\"fontFamily\":\"Soho\",\"padding\":4},\"stacked\":false,\"sort\":true," +
                    "\"ticks\":{\"suggestedMin\":\"0\",\"suggestedMax\":\"1000\",\"maxTicksLimit\":10}," +
                    "\"gridLines\":{\"display\":false}}],\"xAxes\":[{\"stacked\":false,\"sort\":true," +
                    "\"ticks\":{\"fontFamily\":\"Soho\"},\"scaleLabel\":{\"display\":true,\"labelString\":\"\"," +
                    "\"fontFamily\":\"Soho\",\"padding\":4},\"hideLabel\":\"2\",\"gridLines\":{\"display\":true," +
        ...
        int position = 0;
        for (final Iterator<MeasureDTO> iterator = commandDTO.getInformation().getAxes().getMeasuresY().iterator(); iterator.hasNext(); ) {
            final MeasureDTO measureDTOY = iterator.next();
            for (final Iterator<MeasureDTO> iterator2 =
                 commandDTO.getInformation().getAxes().getMeasuresX().iterator(); iterator2.hasNext(); ) {
                final MeasureDTO measureDTOX = iterator2.next();
                final GadgetMeasure measure = new GadgetMeasure();

                final String config =
                        FIELDS_CONCAT + measureDTOX.getPath() + "\",\"" + measureDTOY.getPath() + NAME_CONCAT + measureDTOY.getName() + "\",\"config\": {" + generateMeasureConfig(
                        gadgetType, position, measureDTOY) + "}}";
    ...
    private String generateMeasureConfig(String gadgetType, int position, MeasureDTO measureDTO) {
        ...
        if (gadgetType == null || measureDTO == null) {
            ...
            config = BACKGROUND_COLOR_CONCAT + getColor(position) + BORDER_COLOR_CONCAT + getColor(
                    position) + POINT_BACKGROUND_COLOR_CONCAT + getColor(
                    position) + POINT_HOVER_BACKGROUND_COLOR_CONCAT + getColor(
                    position) + "\",\"yAxisID\":\"#0\",\"fill\":false,\"steppedLine\":false,\"radius\":\"0\"";
        } else if (gadgetType.equals(BAR)) {
            config = BACKGROUND_COLOR_CONCAT + getColor(position) + BORDER_COLOR_CONCAT + getColor(
                    position) + POINT_BACKGROUND_COLOR_CONCAT + getColor(position) + "\",\"yAxisID\":\"#0\"";
        } else if (gadgetType.equals(MIXED)) {
            if (measureDTO.getType() != null && measureDTO.getType().equals(LINE)) {
                config = BACKGROUND_COLOR_CONCAT + getColor(position) + BORDER_COLOR_CONCAT + getColor(
                        position) + POINT_BACKGROUND_COLOR_CONCAT + getColor(
                        position) + POINT_HOVER_BACKGROUND_COLOR_CONCAT + getColor(
                        position) + "\",\"yAxisID\":\"#0\",\"type\":\"line\",\"fill\":false,\"steppedLine\":false," +
                        "\"radius\":\"2\",\"pointRadius\":\"2\",\"pointHoverRadius\":\"2\"";
            } else if (measureDTO.getType() != null && measureDTO.getType().equals(BAR)) {
                config = BACKGROUND_COLOR_CONCAT + getColor(position) + BORDER_COLOR_CONCAT + getColor(
                        position) + POINT_BACKGROUND_COLOR_CONCAT + getColor(
                        position) + POINT_HOVER_BACKGROUND_COLOR_CONCAT + getColor(
                        position) + "\",\"yAxisID\":\"#0\",\"type\":\"bar\",\"fill\":false,\"steppedLine\":false," +
                        "\"radius\":\"1\",\"pointRadius\":\"1\",\"pointHoverRadius\":\"1\"";
            } else {
                config = BACKGROUND_COLOR_CONCAT + getColor(position) + BORDER_COLOR_CONCAT + getColor(
                        position) + POINT_BACKGROUND_COLOR_CONCAT + getColor(
                        position) + POINT_HOVER_BACKGROUND_COLOR_CONCAT + getColor(
                        position) + "\",\"yAxisID\":\"#0\",\"type\":\"points\",\"fill\":false,\"steppedLine\":false," +
    ...
    private List<GadgetMeasure> createGadgetColumns(CommandDTO commandDTO, String gadgetType, User user, Gadget gadget,
        ...
        int position = 0;
        for (final Iterator<MeasureDTO> iterator2 = commandDTO.getInformation().getColumns().iterator(); iterator2.hasNext(); ) {
            final MeasureDTO measureDTO = iterator2.next();
            final GadgetMeasure measure = new GadgetMeasure();
            String config = "{}";
            if (gadgetType != null && gadgetType.equals(WORDCLOUD)) {
                config = FIELDS_CONCAT + measureDTO.getPath() + NAME_CONCAT + measureDTO.getName() + "\",\"config\": {}}";
            } else if (gadgetType != null && gadgetType.equals(TABLE)) {
                config = FIELDS_CONCAT + measureDTO.getPath() + NAME_CONCAT + measureDTO.getName() + "\",\"config\": {\"position\":\"" + position + "\"}}";
            }
            measure.setConfig(config);
            measures.add(measure);
            position++;
    ...
    private List<GadgetMeasure> updateGadgetMeasures(CommandDTO commandDTO) {
        ...
        if (commandDTO.getInformation().getGadgetType().equals(
                ...
                LINE) || commandDTO.getInformation().getGadgetType().equals(
                BAR) || commandDTO.getInformation().getGadgetType().equals(
                MIXED) || commandDTO.getInformation().getGadgetType().equals(RADAR)) {

            for (final Iterator<MeasureDTO> iterator = commandDTO.getInformation().getAxes().getMeasuresY().iterator(); iterator.hasNext(); ) {
                final MeasureDTO measureDTOY = iterator.next();
                for (final Iterator<MeasureDTO> iterator2 = commandDTO.getInformation().getAxes().getMeasuresX().iterator(); iterator2.hasNext(); ) {
                    final MeasureDTO measureDTOX = iterator2.next();
                    final GadgetMeasure measure = new GadgetMeasure();
                    final String config = FIELDS_CONCAT + measureDTOX.getPath() + "\",\"" + measureDTOY.getPath() + NAME_CONCAT + measureDTOY.getName() + "\",\"config\": {" + generateMeasureConfig(
                            commandDTO.getInformation().getGadgetType(), -1, measureDTOY) + "}}";
                    measure.setConfig(config);
                    measures.add(measure);
                }
            ...
            for (final Iterator<MeasureDTO> iterator2 = commandDTO.getInformation().getColumns().iterator(); iterator2.hasNext(); ) {
                ...
                if (commandDTO.getInformation().getGadgetType().equals(WORDCLOUD)) {
                    ...
                    config = FIELDS_CONCAT + measureDTO.getPath() + NAME_CONCAT + measureDTO.getName() + "\",\"config\": {\"position\":\"" + 0 + "\"}}";
                }
                measure.setConfig(config);
                measures.add(measure);
            }
