...
public class DashboardServiceImpl implements DashboardService {
    ...
    private static final String ERROR_SAVING_DASHBOARD_FORBIDDEN = "Cannot update Dashboard that does not exist or " +
    ...
    public List<DashboardDTO> findDashboardWithIdentificationAndDescription(String identification, String description,
        ...
        if (sessionUser.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.toString())) {
            ...
            dashboards =
                    dashboardRepository.findByUserAndPermissionsANDIdentificationContainingAndDescriptionContaining(
    ...
    public String deleteDashboardUserAccess(List<DashboardUserAccessDTO> dtos, String dashboardIdentification,
        ...
        for (DashboardUserAccessDTO dto : dtos) {
            ...
            if (deleteAll) {
                ...
                List<DashboardUserAccessType> accessType = dashboardUserAccessTypeRepository.findByName(
                        dto.getAccessType());
    ...
    public boolean hasUserEditPermission(String id, String userId) {
        ...
        if (user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.toString())) {
            ...
            final DashboardUserAccess userAuthorization = dashboardUserAccessRepository.findByDashboardAndUser(
                    dashboardRepository.findById(id), user);

            if (userAuthorization != null) {
                switch (DashboardUserAccessType.Type.valueOf(
                        userAuthorization.getDashboardUserAccessType().getName())) {
                    case EDIT:
                        return true;
                    case VIEW:
                    default:
    ...
    public String getUserTypePermissionForDashboard(Dashboard dashboard, User user) {
        ...
        if (user.getRole().getId().equals(
                Role.Type.ROLE_ADMINISTRATOR.toString()) || dashboard.getUser().getUserId().equals(user.getUserId())) {
    ...
    private Dashboard getNewDashboard(DashboardCreateDTO dashboard, String userId) {
        ...
        if (sessionUser.getRole().getId().equals(
                Role.Type.ROLE_USER.toString()) || sessionUser.getRole().getId().equals(
                Role.Type.ROLE_DATAVIEWER.toString())) {
    ...
    private void createCategoryRelation(DashboardCreateDTO dashboard, String id) {
        try {
            ...
            final Subcategory subcategory = subcategoryRepository.findByIdentification(dashboard.getSubcategory()).get(
                    0);
    ...
    public String createNewDashboard(DashboardCreateDTO dashboard, String userId) {
        ...
        if (!StringUtils.isEmpty(dashboard.getCategory()) && !StringUtils.isEmpty(
                dashboard.getSubcategory()) && categoryRelationRepository.findByTypeId(dAux.getId()) != null) {
        ...
        try {
            if (dashboard.getAuthorizations() != null) {
                ...
                for (final Iterator<DashboardAccessDTO> iterator = access.iterator(); iterator.hasNext(); ) {
                    ...
                    final List<DashboardUserAccessType> managedTypes = dashboardUserAccessTypeRepository.findByName(
                            dashboardAccessDTO.getAccesstypes());
                    final DashboardUserAccessType managedType =
                            managedTypes != null && !CollectionUtils.isEmpty(managedTypes) ? managedTypes.get(0) : null;
                ...
                if (dashboard.getAuthorizations() != null) {
                    ...
                    for (final Iterator<DashboardAccessDTO> iterator = access.iterator(); iterator.hasNext(); ) {
                        ...
                        final List<DashboardUserAccessType> managedTypes = dashboardUserAccessTypeRepository.findByName(
                                dashboardAccessDTO.getAccesstypes());
                        final DashboardUserAccessType managedType =
                                managedTypes != null && !CollectionUtils.isEmpty(managedTypes) ? managedTypes.get(0)
                                        : null;
                        dua.setDashboardUserAccessType(managedType);
                        dua.setUser(userRepository.findByUserId(dashboardAccessDTO.getUsers()));
                        dashboardUserAccessRepository.save(dua);
    ...
    public String updatePublicDashboard(DashboardCreateDTO dashboard, String userId) {
        if (!dashboardExistsById(dashboard.getId())) {
            ...
            if (dashboard.getCategory() != null && dashboard.getSubcategory() != null && !dashboard.getCategory().isEmpty() && !dashboard.getSubcategory().isEmpty()) {
    ...
    public DashboardExportDTO addGadgets(DashboardExportDTO dashboard) {
        ...
        dashboard.setGadgetDatasources(
                listDatasourcesID.stream().map(this::gadgetDatasourceToDTO).filter(Objects::nonNull).collect(
                        Collectors.toList()));
        dashboard.setGadgetMeasures(
                listGadgetMeasuresID.stream().map(this::gadgetMeasureToDTO).filter(Objects::nonNull).collect(
                        Collectors.toList()));
    ...
    private void addGridBoardFromPage(Map<String, Object> page, List<String> listDatasourcesID,
        ...
        gridboard.forEach(ob -> {
            ...
            if (gadget.containsKey(DATASOURCE)) {
                ...
                final List<GadgetMeasure> gadgetMeasures = gadgetMeasureRepository.findByGadget(
                ...
                if (!CollectionUtils.isEmpty(gadgetMeasures)) {
                    ...
                    final String datasource = gadgetMeasureRepository.findByGadget(
                            gadgetRepository.findById(gadgetId)).get(0).getDatasource().getId();
    ...
    public String importDashboard(DashboardExportDTO dashboardimportDTO, String userId) {
        ...
        if (!dashboardExists(dashboardimportDTO.getIdentification())) {
            ...
            if (!StringUtils.isEmpty(dashboardimportDTO.getCategory()) && !StringUtils.isEmpty(
                    dashboardimportDTO.getSubcategory()) && categoryRelationRepository.findByTypeId(
                    dAux.getId()) != null) {
    ...
    private void includeGadgetDatasoures(DashboardExportDTO dashboard, String userId) {
        for (final GadgetDatasourceDTO gadgetDSDTO : dashboard.getGadgetDatasources()) {
            ...
            if (gadgetDatasourceRepository.findById(gadgetDSDTO.getId()) == null) {
                ...
                if (oDTO.getIdentification() != null && ontologyRepository.findByIdentification(
                        oDTO.getIdentification()) != null)
    ...
    private void processGridboardForElements(Map<String, Object> gadget, JSONArray elements, List<String> added) {
        ...
        if (gadget.containsKey(DATASOURCE)) {
            ...
            elements.put(e);
        } else if (gadget.containsKey("id") && !added.contains(
                gadget.get("id").toString()) && gadgetRepository.findById(gadget.get("id").toString()) != null) {
            ...
            elements.put(e);
            final List<GadgetMeasure> gadgetMeasures = gadgetMeasureRepository.findByGadget(
                    gadgetRepository.findById(gadgetObj.getId()));
            if (!gadgetMeasures.isEmpty()) {

                final GadgetDatasource datasourceObj = gadgetMeasureRepository.findByGadget(
                        gadgetRepository.findById(gadgetObj.getId())).get(0).getDatasource();
    ...
    public ResponseEntity<byte[]> generateImgFromDashboardId(String id, int waittime, int height, int width,
        ...
        UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(
                String.format(TO_IMG, dashboardexporturl)).queryParam("waittime", waittime).queryParam("url",
                                                                                                       prefixURLView + id).queryParam(
                "fullpage", fullpage).queryParam("width", width).queryParam("height", height).queryParam("oauthtoken",
                                                                                                         oauthtoken).queryParam(
                "dashboardsparams", params);
    ...
    public ResponseEntity<byte[]> generatePDFFromDashboardId(String id, int waittime, int height, int width,
        ...
        UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(
                String.format(TO_PDF, dashboardexporturl)).queryParam("waittime", waittime).queryParam("url",
                                                                                                       prefixURLView + id).queryParam(
                "width", width).queryParam("height", height).queryParam("oauthtoken", oauthtoken).queryParam(
                "dashboardsparams", params);
