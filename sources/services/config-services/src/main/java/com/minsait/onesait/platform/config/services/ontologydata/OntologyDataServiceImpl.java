...
public class OntologyDataServiceImpl implements OntologyDataService {
    ...
    private static final String REQ_SAME_SCH = "Schema can't be modified in this type of ontology, please delete all " +
            "data and then change it";

    private final DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss'Z'").withZone(
            ZoneId.of("UTC"));
    ...
    String addContextData(final OperationModel operationModel, JsonNode data) throws IOException {
        ...
        final ContextData contextData = ContextData.builder(user, timezoneId, timestamp, timestampMillis,
                                                            source).clientConnection(clientConnection).deviceTemplate(
                deviceTemplate).device(device).clientSession(clientSession).build();
    ...
    String encryptionOperationAllowingArrays(String data, Ontology ontology,
            EncryptionOperations operation) throws IOException {
    ...
    JsonNode encryptionOperation(JsonNode jsonData, Ontology ontology,
            EncryptionOperations operation) throws IOException {
    ...
    public void processPropertiesForTitleCase(String field, JsonNode value, JsonNode root, String pointer) {
        if (!value.path("$ref").isMissingNode()) {
            ...
            root.at(newPointer + "/" + PROP_STR).fields().forEachRemaining(
    ...
    private void processSubPropertiesForTilteCase(String field, JsonNode value, JsonNode root, String pointer) {
        ...
        if (type.equalsIgnoreCase("object")) {
            ...
            root.at(newPointer).fields().forEachRemaining(
                    e -> processPropertiesForTitleCase(e.getKey(), e.getValue(), root, newPointer));
        ...
        if (ontology != null) {
            ...
            try {
                ...
                final String parentNode =
                        reference.equals("") ? PROP_STR + "." : reference.replace("/", "") + ".properties.";
