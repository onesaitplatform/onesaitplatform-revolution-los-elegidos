...
public class KsqlRelationServiceImpl implements KsqlRelationService {
    ...
    public void createKsqlRelation(KsqlFlow ksqlFlow, KsqlResource ksqlResource) throws KsqlExecutionException {
        ...
        KsqlResource existingKsqlResource = ksqlResourceService.getKsqlResourceByIdentification(
        ...
        if (existingKsqlResource == null) {
            ...
            if (ksqlResource.getResourceType() == FlowResourceType.PROCESS) {
                ...
                throw new KsqlRelationServiceException(
                        "Resource with the same IDENTIFICARION already exists. KsqlFlow=" + ksqlFlow.getIdentification() + ", " + KSQL_RESOURCE_STRING + "=" + ksqlResource.getIdentification());
            }
            if (ksqlRelationRepository.findByKsqlFlowAndKsqlResource(ksqlFlow, ksqlResource) != null) {
                log.error("Relation already exists. KsqlFlow=%s, KsqlResource=%s.", ksqlFlow.getIdentification(),
                          ksqlResource.getIdentification());
                throw new KsqlRelationServiceException(
                        "Relation already exists. KsqlFlow=" + ksqlFlow.getIdentification() + ", " + KSQL_RESOURCE_STRING + "=" + ksqlResource.getIdentification());
            }
        }
        ...
        for (String dependency : dependencies) {
            List<KsqlRelation> predecessors = ksqlRelationRepository.findByKsqlFlowAndKsqlResourceIdentification(
                    ksqlFlow, dependency);
    ...
    public void deleteKsqlRelation(KsqlRelation relation) throws KsqlExecutionException {
        ...
        if (ksqlResource.getResourceType() == FlowResourceType.ORIGIN) {
            List<KsqlRelation> relations = ksqlRelationRepository.findByKsqlResourceIdentification(
                    ksqlResource.getIdentification());
    ...
    @Override
    @Transactional
    public void updateKsqlRelation(KsqlFlow ksqlFlow, KsqlResource ksqlResource, String statement,
            String description) throws KsqlExecutionException {
        ...
        if (relation == null) {
            log.error("Unable to find Relation for the Resource Update command. KsqlFlow = {}, KsqlResource = {}.",
                      ksqlFlow.getIdentification(), ksqlResource.getIdentification());
            throw new KsqlRelationServiceException(
                    "Unable to find Relation for the Resource Update command. KsqlFlow = " + ksqlFlow.getIdentification() + ", " + KSQL_RESOURCE_STRING + " = " + ksqlResource.getIdentification());
        }
    ...
    public List<KsqlRelation> getKsqlRelationsWithFlowIdDescriptionAndIdentification(User sessionUser, String id,
        ...
        if (sessionUser.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.toString())) {
            ksqlRelations =
                    ksqlRelationRepository.findByKsqlFlowIdAndKsqlResourceIdentificationContainingAndKsqlResourceDescriptionContaining(
                    id, identification, description);
        } else {
            ksqlRelations = ksqlRelationRepository.findByKsqlFlowUserAndKsqlFlowIdAndKsqlResourceIdentificationContainingAndKsqlResourceDescriptionContaining(
    ...
    private void checkTopicAvailability(KsqlFlow ksqlFlow, KsqlResource ksqlResource) {
        if (ksqlResource.getResourceType() == FlowResourceType.PROCESS) {
            List<KsqlRelation> relations = ksqlRelationRepository.findByKsqlResourceKafkaTopicAndKsqlFlowNot(
                    ksqlResource.getKafkaTopic(), ksqlFlow);
            if (relations != null && !relations.isEmpty()) {
                log.error("Specified TOPIC already in use in a different FLOW. KsqlFlow=%s, KsqlResource=%s.",
                          ksqlFlow.getIdentification(), ksqlResource.getIdentification());
                throw new KsqlRelationServiceException(
                        "Specified TOPIC already in use in a different FLOW. KsqlFlow=" + ksqlFlow.getIdentification() + ", KsqlResource=" + ksqlResource.getIdentification());
            }
        }
    }
