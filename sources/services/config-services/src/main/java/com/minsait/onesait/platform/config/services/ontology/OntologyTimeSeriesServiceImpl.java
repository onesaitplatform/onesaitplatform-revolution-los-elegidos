...
public class OntologyTimeSeriesServiceImpl implements OntologyTimeSeriesService {
    ...
    public ResponseEntity<Map<String, String>> createOntologyTimeSeries(OntologyTimeSeriesDTO ontologyTimeSeriesDTO,
        ...
        if (ontologyTimeSeriesDTO.isStats()) {
            ...
            stats.setJsonSchema(
                    "{\"$schema\":\"http://json-schema.org/draft-07/schema#\",\"title\":\"" + ontologyTimeSeriesDTO.getIdentification() + STATS_STR + "\",\"type\":\"object\",\"required\":[\"" + "Stats" + "\"],\"properties\":{\"" + ontologyTimeSeriesDTO.getIdentification() + STATS_STR + "\":{\"type\":\"string\",\"$ref\":\"#/datos\"}},\"datos\":{\"description\":\"Properties for DataModel\",\"type\":\"object\",\"required\":[\"tag\",\"field\",\"lastValue\",\"windowType\",\"windowFreq\"],\"properties\":{\"tag\":{\"type\":\"array\", \"items\": [{\"type\":\"object\", \"properties\": {\"name\":{\"type\":\"string\"}, \"value\":{\"type\":\"object\"} }}], \"minItems\":1},\"field\":{\"type\":\"string\"},\"lastValue\":{\"type\":\"object\"},\"windowType\":{\"type\":\"string\"},\"windowFreq\":{\"type\":\"string\"}}},\"description\":\"" + "Stats for Ontology " + ontologyTimeSeriesDTO.getIdentification() + "\",\"additionalProperties\":true}");
    ...
    public ResponseEntity<?> updateOntologyTimeSeries(OntologyTimeSeriesDTO ontologyTimeSeriesDTO, String sessionUserId,
        ...
        Set<OntologyTimeSeriesProperty> ontologyTSPDb = ontologyTimeSeriesPropertyRepository.findByOntologyTimeSeries(
                oTS);
        ...
        for (final OntologyTimeSeriesProperty oTSP : ontologyTimeSeriesDTO.getTimeSeriesProperties()) {
            for (final OntologyTimeSeriesProperty oTSPDB : ontologyTSPDb) {
                if (oTSP.getPropertyType().equals(oTSPDB.getPropertyType()) && oTSP.getPropertyDataType().equals(
                        oTSPDB.getPropertyDataType()) && oTSP.getPropertyName().equals(oTSPDB.getPropertyName())) {
                    flag = false;
                }
            }
            oTSP.setOntologyTimeSeries(oTS);
        ...
        for (final OntologyTimeSeriesWindow oTSW : ontologyTimeSeriesDTO.getTimeSeriesWindow()) {
            for (final OntologyTimeSeriesWindow oTSWDB : ontologyTSWDb) {
                if (oTSW.getWindowType().equals(oTSWDB.getWindowType()) && oTSW.getFrecuencyUnit().equals(
                        oTSWDB.getFrecuencyUnit()) && oTSW.getFrecuency().equals(
                        oTSWDB.getFrecuency()) && oTSW.getAggregationFunction().equals(
                        oTSWDB.getAggregationFunction()) && oTSW.isBdh() == oTSWDB.isBdh()) {
                    flag = false;
                }
            }
            if (flag) {
