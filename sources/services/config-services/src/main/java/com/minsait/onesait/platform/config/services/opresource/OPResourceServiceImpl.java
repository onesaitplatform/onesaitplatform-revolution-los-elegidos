...
public class OPResourceServiceImpl implements OPResourceService {
    ...
    public Collection<OPResource> getResources(String userId, String identification) {
        ...
        if (user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.name()))
            ...
            final List<OPResource> resources = resourceRepository.findByIdentificationContainingIgnoreCase(
                    identification);
            final Set<OPResource> resourcesFiltered = resources.stream().filter(r -> r.getUser().equals(user)).collect(
                    Collectors.toSet());
            resourcesFiltered.addAll(resources.stream().filter(r -> r instanceof Ontology).filter(
                    o -> ((Ontology) o).getOntologyUserAccesses().stream().map(
                            a -> (a.getUser().equals(user) && a.getOntologyUserAccessType().getName().equals(
                                    OntologyUserAccessType.Type.ALL.name()))).findAny().orElse(false)).collect(
                    Collectors.toSet()));
    ...
    public void createUpdateAuthorization(ProjectResourceAccess pRA) {
        ...
        if (pRA.getAppRole() != null)
            pRADB = pRA.getProject().getProjectResourceAccesses().stream().filter(
                    a -> a.getResource().equals(pRA.getResource()) && a.getAppRole().equals(
                            pRA.getAppRole()) && a.getProject().equals(pRA.getProject())).findFirst().orElse(null);

        else
            pRADB = pRA.getProject().getProjectResourceAccesses().stream().filter(
                    a -> a.getResource().equals(pRA.getResource()) && a.getUser().equals(
                            pRA.getUser()) && a.getProject().equals(pRA.getProject())).findFirst().orElse(null);
    ...
    public boolean hasAccess(String userId, String resourceId, ResourceAccessType access) {
        ...
        return accesses.stream().map(pra -> userIsAllowed(pra, user, access)).filter(
                Boolean::booleanValue).findFirst().orElse(false);
    }

    private boolean userIsAllowed(ProjectResourceAccess pra, User user, ResourceAccessType access) {
        if (pra.getAppRole() != null) {
            final User userInApp = pra.getAppRole().getAppUsers().stream().map(AppUser::getUser).filter(
                    u -> u.equals(user)).findFirst().orElse(null);
            if (userInApp != null) {
                switch (access) {
                    case MANAGE:
    ...
    public void insertAuthorizations(Set<ProjectResourceAccess> accesses) {
        ...
        final Set<ProjectResourceAccess> repeated = accesses.stream().filter(
                pra -> project.getProjectResourceAccesses().contains(pra)).collect(Collectors.toSet());
    ...
    public Collection<OPResource> getResourcesForUserAndType(User user, String type) {
        ...
        if (!CollectionUtils.isEmpty(appUsers)) {
            appUsers.forEach(au -> resources.addAll(resourceAccessRepository.findByAppRole(au.getRole()).stream().map(
                    ProjectResourceAccess::getResource).filter(r -> r.getClass().getSimpleName().equals(type)).collect(
                    Collectors.toSet())));
        }
        resources.addAll(
                resourceAccessRepository.findByUser(user).stream().map(ProjectResourceAccess::getResource).filter(
                        r -> r.getClass().getSimpleName().equals(type)).collect(Collectors.toSet()));
        return resources;
    }
    ...
    public boolean isUserAuthorized(String userId, String resourceId) {
        ...
        return (user.getRole().getId().equals(
                Role.Type.ROLE_ADMINISTRATOR.name()) || (resource != null && resource.getUser().equals(
                user)) || hasAccess(userId, resourceId, ResourceAccessType.MANAGE));
