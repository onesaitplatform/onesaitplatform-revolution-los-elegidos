...
public class NotebookServiceImpl implements NotebookService {
    ...
    private static final String WARNING_NOT_POSSIBLE_EXTRACT_OSP_INFO = "WARN: Not possible to extract ospInfo from " +
            "notebook";
    private static final String ERROR_USER_CANNOT_OVERWRITE_NOTEBOOK = "User %s not authorized to overwrite notebook " +
            "%s";
    private static final String ERROR_NOTEBOOK_NOT_FOUND_ZEPPELIN = "Notebook not found in zeppelin";
    private static final String HAS_NOT_PERMISSION_TO_RESTART_INTERPRETER = " has not permission to restart " +
            "interpreter ";
        ...
        if (existNotebookIdentification(name) && overwrite) {
            ...
            if (ntExists.getUser().equals(user) || isUserAdmin) {
                ...
                throw new NotebookServiceException(Error.PERMISSION_DENIED,
                                                   String.format(ERROR_USER_CANNOT_OVERWRITE_NOTEBOOK, user.getUserId(),
                                                                 ntExists.getIdentification()));
            }
    ...
    private Notebook updateNotebook(String notebookId, String userId, String data, boolean importAuthorizations) {
        ...
        try {
            ...
            log.warn(
                    "Not possible to delete deprecated notebook " + nameOld + "(" + idzepOld + ") after update " +
                            "notebook: " + responseEntity,
                    e);
        }
        return nt;
    ...
    public JSONObject exportNotebook(String id, String user) {
        ...
        if (hasUserPermissionInNotebook(nt, user)) {
            ...
            try {
                ...
                if (isUserOwnerOfNotebook(userObj, nt) || userObj.getRole().getId().equals(
    ...
    @Override
    public ResponseEntity<String> sendHttp(HttpServletRequest requestServlet, HttpMethod httpMethod,
            String body) throws URISyntaxException, IOException {
        return sendHttp(requestServlet.getServletPath(), httpMethod, body);
    }

    @Override
    public ResponseEntity<String> sendHttp(String url, HttpMethod httpMethod,
            String body) throws URISyntaxException, IOException {
        final HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
    ...
    private boolean hasUserPermissionCreateNotebook(User user) {
        return (user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.toString()) || user.getRole().getId().equals(
                Role.Type.ROLE_DATASCIENTIST.toString()));
    }
    ...
    private boolean hasUserPermissionInNotebook(Notebook nt, User user) {
        if (user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.toString()) || nt.getUser().getUserId().equals(
                user.getUserId()) || nt.isPublic())
            return true;
    ...
    @Override
    public ResponseEntity<String> getParagraphResult(String zeppelinId,
            String paragraphId) throws URISyntaxException, IOException {
    ...
    public List<String> createUserAccess(String notebookId, List<String> userIds, List<String> accessTypes) {
        ...
        for (int i = 0; i < userIds.size(); i++) {
            try {
                ...
                NotebookUserAccessType accessType = notebookUserAccessTypeRepository.findUserAccessTypeByName(
                        accessTypes.get(i).toUpperCase());
    ...
    public List<String> deleteUserAccess(String notebookId, List<String> userIds, List<String> accessTypes) {
        ...
        for (int i = 0; i < userIds.size(); i++) {
            try {
                ...
                NotebookUserAccess notebookAccess = notebookUserAccessRepository.findByNotebookAndUserAndAccess(nt,
                                                                                                                user,
                                                                                                                accessType);
    ...
    private void rename(String name, Notebook nt) {
		...
		/*ResponseEntity<String> rstr = sendHttp(API_NOTEBOOK_STR.concat(nt.getIdzep()).concat("/rename"), HttpMethod
		.PUT, NAME_STR + name + "'}");
    ...
    private ResponseEntity<String> restartInterpreter(String interpreterName, String interpreterId,
            String body) throws URISyntaxException, IOException {
    ...
    @Override
    public ResponseEntity<String> restartInterpreter(String interpreterName,
            String body) throws URISyntaxException, IOException {
    ...
    public ResponseEntity<String> restartInterpreter(String interpreterName, String body,
            User user) throws URISyntaxException, IOException {
        ResponseEntity<String> responseEntity;

        try {

            if (user.getRole().getId().equals(
                    Role.Type.ROLE_ADMINISTRATOR.toString()) || (user.getRole().getId().equals(
                    Role.Type.ROLE_DATASCIENTIST.toString()) && !isSharedInterpreterConfiguration(interpreterName))) {
                ...
                JSONObject responseBody = new JSONObject().put("error",
                                                               USER2 + user.getUserId() + HAS_NOT_PERMISSION_TO_RESTART_INTERPRETER + interpreterName + ". Check interpreter configuration.");
    ...
    private boolean isSharedInterpreterConfiguration(String interpreterName,
            String interpreterId) throws URISyntaxException, IOException {
    ...
    @Override
    public ResponseEntity<String> restartAllInterpretersNotebook(String notebookId, String body,
            User user) throws URISyntaxException, IOException {
        ...
        for (Map.Entry<String, String> entry : notebookInterpreters.entrySet()) {
            ...
            try {
                if (user.getRole().getId().equals(
                        Role.Type.ROLE_ADMINISTRATOR.toString()) || (user.getRole().getId().equals(
                        Role.Type.ROLE_DATASCIENTIST.toString()) && !isSharedInterpreterConfiguration(entry.getKey(),
                                                                                                      entry.getValue()))) {
                    ...
                    log.error(
                            USER2 + user.getUserId() + HAS_NOT_PERMISSION_TO_RESTART_INTERPRETER + entry.getKey() + ". Check interpreter configuration.");
            ...
            for (int i = 0; i < paragraphs.length(); i++) {
                ...
                if (!object.isNull("text")) {
                    ...
                    if (firstLine.startsWith("%")) {
                        ...
                        if (!notebookInterpreters.containsKey(nameInterpreter) && listInterpreters.containsKey(
                                nameInterpreter)) {
                            notebookInterpreters.put(nameInterpreter, listInterpreters.get(nameInterpreter));
                        } else if (!notebookInterpreters.containsKey(
                                nameInterpreter) && !notebookInterpreters.containsKey(defaultInterpreter)) {
