...
public class ProjectServiceImpl implements ProjectService {
    ...
    private List<ProjectUserDTO> getMembersDTO(List<?> members) {
        return members.stream().map(o -> {
            if (o instanceof AppUser)
                return ProjectUserDTO.builder().userId(((AppUser) o).getUser().getUserId()).roleName(
                        ((AppUser) o).getRole().getName()).fullName(((AppUser) o).getUser().getFullName()).realm(
                        ((AppUser) o).getRole().getApp().getAppId()).build();
            else if (o instanceof User)
                return ProjectUserDTO.builder().userId(((User) o).getUserId()).roleName(
                        ((User) o).getRole().getId()).fullName(((User) o).getFullName()).build();
            else
                return null;
    ...
    public List<App> getAvailableRealmsForUser(String userId) {
        return appService.getAppsByUser(userId, null).stream().filter(a -> a.getProject() == null).collect(
                Collectors.toList());
    }
    ...
    public List<Project> getProjectsForUser(String userId) {
        ...
        if (user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.name()))
            return projects;
        final List<Project> filteredProjects = projects.stream().filter(
                p -> p.getUser().equals(user) || p.getUsers().contains(user)).collect(Collectors.toList());
        ...
        if (p.getApp() != null) {
            ...
            if (!CollectionUtils.isEmpty(app.getChildApps())) {
                app.getChildApps().forEach(a -> a.getAppRoles().forEach(r -> {
                    final List<User> users = r.getAppUsers().stream().map(AppUser::getUser).collect(
                            Collectors.toList());
    ...
    public boolean isUserInProject(String userId, String projectId) {
        ...
        if (project.getUser().equals(user))
            ...
            return project.getApp().getAppRoles().stream().map(
                    ar -> ar.getAppUsers().stream().map(au -> au.getUser().equals(user)).filter(
                            Boolean::booleanValue).findFirst().orElse(false)).filter(
    ...
    public Set<ProjectResourceAccess> getResourcesAccessesForUser(String projectId, String userId) {
        ...
        final Project project = projectRepository.findOne(projectId);
        if (project.getApp() != null) {
            final Set<AppRole> roles = project.getApp().getAppRoles().stream().filter(
                    ar -> null != ar.getAppUsers().stream().map(AppUser::getUser).filter(
                            u -> u.equals(user)).findFirst().orElse(null)).collect(Collectors.toSet());
            return project.getProjectResourceAccesses().stream().filter(
                    pra -> roles.contains(pra.getAppRole())).collect(Collectors.toSet());
        } else {
            return project.getProjectResourceAccesses().stream().filter(pra -> pra.getUser().equals(user)).collect(
                    Collectors.toSet());
        }
    }
    ...
    public Set<OPResource> getResourcesForProjectAndUser(String projectId, String userId) {
        ...
        if (user.equals(project.getUser()) || role_user.equals(Role.Type.ROLE_ADMINISTRATOR.toString()))
            return project.getProjectResourceAccesses().stream().map(ProjectResourceAccess::getResource).collect(
                    Collectors.toSet());
        else
            return getResourcesAccessesForUser(projectId, userId).stream().map(
                    ProjectResourceAccess::getResource).collect(Collectors.toSet());
    }
    ...
    public <T> Set<T> getResourcesForUserOfType(String userId, Class<T> clazz) {
        return getResourcesForUser(userId).stream().filter(clazz::isInstance).map(clazz::cast).collect(
                Collectors.toSet());
    }
