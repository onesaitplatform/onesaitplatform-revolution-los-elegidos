...
public class TwitterListeningServiceImpl implements TwitterListeningService {
    ...
    public TwitterListening createListening(TwitterListening twitterListening, String userSessionId) {
        if (twitterListening.getOntology().getId() == null)
            twitterListening.setOntology(
                    this.ontologyService.getOntologyByIdentification(twitterListening.getOntology().getIdentification(),
                                                                     userSessionId));
        ...
        if (twitterListening.getConfiguration().getId() == null)
            twitterListening.setConfiguration(this.configurationService.getConfigurationByDescription(
                    twitterListening.getConfiguration().getDescription()));
    ...
    public void updateListening(TwitterListening twitterListening) {
        ...
        if (newTwitterListening != null) {
            ...
            newTwitterListening.setConfiguration(this.configurationService.getConfigurationByDescription(
                    twitterListening.getConfiguration().getDescription()));
