...
public class BinaryFileServiceImpl implements BinaryFileService {
    ...
    public boolean hasUserPermissionWrite(String id, User user) {
        return (user.getRole().getId().equals(
                Role.Type.ROLE_ADMINISTRATOR.name()) || binaryFileRepository.findByUserAndFileIdWrite(user,
                                                                                                      id) != null);
    }
    ...
    public boolean hasUserPermissionRead(String id, User user) {
        if (user == null) {
            ...
            return (user.getRole().getId().equals(
                    Role.Type.ROLE_ADMINISTRATOR.name()) || binaryFileRepository.findByUserAndFileId(user, id) != null);
        }
    }
    ...
    public boolean isUserOwner(String fileId, User user) {
        final BinaryFile file = binaryFileRepository.findByFileId(fileId);
        return (file.getOwner().getUserId().equalsIgnoreCase(user.getUserId()) || user.getRole().getId().equals(
                Role.Type.ROLE_ADMINISTRATOR.name()));
    }

    @Override
    public BinaryFileAccess createBinaryFileAccess(String fileId, String userId, String accessType,
            User user) throws GenericOPException {
