...
public class KsqlFlowServiceImpl implements KsqlFlowService {
    ...
    public void deleteKsqlFlow(String id) throws KsqlExecutionException {
        ...
        List<KsqlRelation> process =
                ksqlRelationRepository.findByKsqlFlowAndKsqlResourceResourceTypeAndKsqlResourceKsqlTypeNotOrderByCreatedAtDesc(
                flow, FlowResourceType.PROCESS, KsqlResourceType.INSERT);
        // THIRD delete all DESTINY
        List<KsqlRelation> destiny =
                ksqlRelationRepository.findByKsqlFlowAndKsqlResourceResourceTypeOrderByCreatedAtDesc(
                flow, FlowResourceType.DESTINY);
        // LAST delete all ORIGIN
        List<KsqlRelation> origin =
                ksqlRelationRepository.findByKsqlFlowAndKsqlResourceResourceTypeOrderByCreatedAtDesc(
                flow, FlowResourceType.ORIGIN);
        ...
        for (KsqlRelation element : elements) {
            ...
            if (relationResource.getResourceType() != FlowResourceType.ORIGIN) {
                ...
                // then remove it
                List<KsqlRelation> otherRelations = ksqlRelationRepository.findByKsqlResourceIdentification(
                        relationResource.getIdentification());
    ...
    public void createKsqlFlow(KsqlFlow ksqlFlow) {
        if (identificationIsAvailable(ksqlFlow.getUser(), ksqlFlow.getIdentification())) {
            ...
            throw new KsqlFlowServiceException(
                    "Identification is already being used for a previous KSQL Flow. Identification = " + ksqlFlow.getIdentification());
        }
    }
