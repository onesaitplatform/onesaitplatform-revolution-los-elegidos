...
public class DataflowServiceImpl implements DataflowService {
    ...
    @Override
    public Pipeline createPipeline(String name, PipelineTypes type, String description,
        ...
        if (pipelineRepository.findByIdentification(name) != null) {
            ...
            return sendStreamsetsCreatePut("rest/v1/pipeline/" + UriUtils.encode(name,
                                                                                 UTF8_STR) + "?autoGeneratePipelineId" +
                                                   "=true&pipelineType=" + type.toString() + "&description=" + description,
                                           name, userId);
        }
    }
    ...
    public ResponseEntity<String> sendHttp(HttpServletRequest requestServlet, HttpMethod httpMethod, Object body,
        ...
        return sendHttp(requestServlet.getServletPath() + (requestServlet.getQueryString() != null
                                ? "?" + requestServlet.getQueryString() : ""), httpMethod, body, user,
                        requestServlet.getContentType() == null ? MediaType.APPLICATION_JSON.toString()
                                : requestServlet.getContentType(), requestServlet.getHeader(DATAFLOW_HEADER));
    }
    ...
    private boolean hasUserPermissionInPipeline(Pipeline pl, User user) {
        final PipelineUserAccessType pipelineUserAccessType = pipelineUserAccessTypeRepository.findById(
                "ACCESS-TYPE-1");
        if (user == null || pl == null)
            return false;
        return user.getRole().getId().equals(
                Role.Type.ROLE_ADMINISTRATOR.toString()) || pl.getUser().getUserId().equals(
                user.getUserId()) || pipelineUserAccessRepository.findByPipelineAndUserAndAccess(pl, user,
                                                                                                 pipelineUserAccessType) != null || pl.isPublic();
    }
    ...
    public boolean hasUserViewPermission(String pipelineId, String userId) {
        final Pipeline pl = pipelineRepository.findByIdstreamsets(pipelineId);
        return hasUserPermissionForPipeline(pipelineId, userId) || resourceService.hasAccess(userId, pl.getId(),
    ...
    public ResponseEntity<String> pipelines(String userId, String filterText, String label, int offset, int len,
        ...
        final HttpHeaders headers = StreamsetsApiWrapper.setBasicAuthorization(new HttpHeaders(), dataflowUser,
                                                                               dataflowPass);
        final ResponseEntity<String> response = StreamsetsApiWrapper.pipelines(rt, headers, dataflowServiceUrl,
                                                                               filterText, label, offset, len, orderBy,
                                                                               order, includeStatus);
        final String body = response.getBody();
    ...
    public void createUserAccess(String dataflowId, String userId, String accessType) {
        if (dataflowId != null && !dataflowId.equals("") && userId != null && !userId.equals(
