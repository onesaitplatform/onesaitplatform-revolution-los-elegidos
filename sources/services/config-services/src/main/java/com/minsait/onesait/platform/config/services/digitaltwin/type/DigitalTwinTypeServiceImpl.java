...
public class DigitalTwinTypeServiceImpl implements DigitalTwinTypeService {
    ...
    public List<PropertyDigitalTwinTypeDTO> getPropertiesByDigitalId(String typeId) {
        ...
        final List<PropertyDigitalTwinType> lProperties = propDigitalTwinTypeRepo.findByTypeId(
                digitalTwinTypeRepo.findById(typeId));

        for (final PropertyDigitalTwinType prop : lProperties) {
            lPropertiesDTO.add(
                    new PropertyDigitalTwinTypeDTO(prop.getId(), prop.getType(), prop.getName(), prop.getUnit(),
                                                   prop.getDirection(), prop.getDescription()));
        }
    ...
    public List<ActionsDigitalTwinTypeDTO> getActionsByDigitalId(String typeId) {
        ...
        final List<ActionsDigitalTwinType> lActions = actDigitalTwinTypeRepo.findByTypeId(
                digitalTwinTypeRepo.findById(typeId));
    ...
    public List<EventsDigitalTwinTypeDTO> getEventsByDigitalId(String typeId) {
        ...
        final List<EventsDigitalTwinType> lEvents = evtDigitalTwinTypeRepo.findByTypeId(
                digitalTwinTypeRepo.findById(typeId));

        for (final EventsDigitalTwinType event : lEvents) {
            if (!event.getType().equalsIgnoreCase(
                    EventsDigitalTwinType.Type.PING.name()) && !event.getType().equalsIgnoreCase(
                    EventsDigitalTwinType.Type.REGISTER.name())) {
                lEventsDTO.add(
                        new EventsDigitalTwinTypeDTO(event.getId(), event.getType(), event.getName(), event.isStatus(),
    ...
    public void updateDigitalTwinType(DigitalTwinType digitalTwinType, HttpServletRequest httpServletRequest) {
        ...
        if (digitalTwinTypeDb != null) {
            ...
            log.error(
                    "DigitalTwinType with identification:" + digitalTwinType.getName() + "don't exist in data base to" +
                            " update.");
        }
