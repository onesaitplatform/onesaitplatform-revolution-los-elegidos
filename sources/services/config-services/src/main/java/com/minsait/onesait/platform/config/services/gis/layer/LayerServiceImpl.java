...
public class LayerServiceImpl implements LayerService {
    ...
    public Layer findById(String id, String userId) {
        ...
        if (user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.toString()) || layer.getUser().equals(
                user) || layer.isPublic()) {
            return layer;
        } else {
    ...
    @Override
    public Map<String, String> getOntologyGeometryFields(String identification,
        ...
        if (ontology != null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (jsonNode.path(property).get("type").asText().equals("object")) {
                    ...
                    if (!jsonNodeAux.path("coordinates").isMissingNode() && jsonNodeAux.path("coordinates").get(
                            "type").asText().equals("array")) {
                        hasCoordinates = true;
                    }
                    if (!jsonNodeAux.path("type").isMissingNode() && !jsonNodeAux.path("type").path(
                            "enum").isMissingNode() && jsonNodeAux.path("type").get("enum").isArray()) {
                        hasType = true;
                    }
    ...
    public Layer getLayerByIdentification(String identification, User user) {
        ...
        if (!layers.isEmpty() && (user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.toString()) || layers.get(
                0).getUser().equals(user))) {
            return layers.get(0);
    ...
    public String getLayerSvgImage(String layerIdentification) {
        Layer layer = layerRepository.findByIdentification(layerIdentification).get(0);
        return URL_CONCAT + layer.getUrl() + "\",\"west\":\"" + layer.getWest() + "\" ,\"east\":\"" + layer.getEast() + "\",\"north\":\"" + layer.getNorth() + "\",\"south\":\"" + layer.getSouth() + "\"}";
    }
