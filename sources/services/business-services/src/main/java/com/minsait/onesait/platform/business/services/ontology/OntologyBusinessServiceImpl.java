...
public class OntologyBusinessServiceImpl implements OntologyBusinessService {
    ...
    @Override
    public void createOntology(Ontology ontology, String userId,
            OntologyConfiguration config) throws OntologyBusinessServiceException {
    ...
    private void invokeCreateOntology(Ontology ontology, OntologyConfiguration config,
        ...
        try {
            ...
            throw new OntologyBusinessServiceException(OntologyBusinessServiceException.Error.CONFIG_CREATION_ERROR,
                                                       "Error creating the ontology configuration: " + e.getMessage(),
                                                       e);
        }
        ...
        try {
            ...
            try {
                ...
                throw new OntologyBusinessServiceException(
                        ...
                        "Error creating the persistence infrastructure for ontology, it was not possible to undo the " +
                                "ontology configuration and/or the kafka topic creation");
            }
    ...
    public JsonNode organizeRootNodeIfExist(String schema) throws IOException {
        ...
        for (final Iterator<Entry<String, JsonNode>> elements = schemaSubTree.fields(); elements.hasNext(); ) {
            ...
            if (e.getKey().equals("properties")) {
                e.getValue().fields();
                for (final Iterator<Entry<String, JsonNode>> properties = e.getValue().fields(); properties.hasNext(); ) {
    ...
    @Override
    public void updateOntology(Ontology ontology, OntologyConfiguration config,
            boolean hasDocuments) throws OntologyBusinessServiceException {
        if (ontology.getRtdbDatasource().equals(RtdbDatasource.KUDU)) {
            if (!hasDocuments) {
                ...
                try {
                    ...
                    throw new OntologyBusinessServiceException(
                            OntologyBusinessServiceException.Error.CONFIG_CREATION_ERROR,
                            "Error cannot update internal config with data in ontology, please remove data and update it again. The error is the following: " + e.getMessage(),
                            e);
                }
            }
        }
