...
public class TwitterServiceTest {
    ...
    @Before
    public void setup() {
        twitterS = TwitterServiceFactory.getSpringSocialImpl(consumerKey, consumerSecret, accessToken,
                                                             accessTokenSecret);
    }
    ...
    public void given_twitterService_When_ComplexSearchesWithServeralParametersAreRequested_Then_TheResponseOntainTweets() {
        SearchParameters params = new SearchParameters("madrid").geoCode(
                new GeoCode(52.379241, 4.900846, 100, GeoCode.Unit.MILE)).lang("es").resultType(
                SearchParameters.ResultType.RECENT).count(25).includeEntities(false);
