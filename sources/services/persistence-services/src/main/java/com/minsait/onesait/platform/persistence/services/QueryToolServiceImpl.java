...
public class QueryToolServiceImpl implements QueryToolService {
    ...
    private void hasUserPermission(String user, String ontology, String query) {
        ...
        if (isJoin(query)) {
            try {
                SQLParser.getTables(query).stream().forEach(s -> {
                    ...
                    if (destination == null || source == null || !source.getRtdbDatasource().equals(
                            destination.getRtdbDatasource()))
        ...
        if ((query.toLowerCase().indexOf("update") != -1 || query.toLowerCase().indexOf(
                "remove") != -1 || query.toLowerCase().indexOf("delete") != -1 || query.toLowerCase().indexOf(
                "createindex") != -1) && !ontologyService.hasUserPermissionForInsert(user, ontology)) {
            throw new DBPersistenceException(
    ...
    public String queryNativeAsJsonForPlatformClient(String clientPlatform, String ontology, String query, int offset,
        ...
        try {
            ...
            String result = queryAsTextDBRepositoryFactory.getInstanceClientPlatform(ontology,
                                                                                     clientPlatform).queryNativeAsJson(
                    ontology, query, offset, limit);
        ...
        try {
            ...
            if (checkTemplates) {
                ...
                if (newQuery != null) {
                    ...
                    switch (newQuery.getType()) {
                        case SQL:
                            return querySQLAsJsonForPlatformClient(clientPlatform, ontology, newQuery.getQuery(),
                                                                   offset, false);
                        case NATIVE:
                            return queryNativeAsJsonForPlatformClient(clientPlatform, ontology, newQuery.getQuery(),
                                                                      offset, 0);
                        default:
                            throw new IllegalStateException("Only SQL or NATIVE queries are supported");
                    }
            ...
            query = QueryParsers.parseFunctionNow(query);
            return queryAsTextDBRepositoryFactory.getInstanceClientPlatform(ontology, clientPlatform).querySQLAsJson(
                    ontology, query, offset);
        } catch (final Exception e) {
    ...
    @Override
    public String querySQLAsJsonForPlatformClient(String clientPlatform, String ontology, String query,
            int offset) throws DBPersistenceException {
