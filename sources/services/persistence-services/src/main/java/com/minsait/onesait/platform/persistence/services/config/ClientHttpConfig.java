...
public class ClientHttpConfig {
    ...
    HttpClient httpClient() throws GenericOPException {
        ...
        final RequestConfig requestConfig = RequestConfig.custom().setConnectionRequestTimeout(
                getTimeout()).setConnectTimeout(getTimeout()).setSocketTimeout(getTimeout()).build();
    ...
    private int getTimeout() {
        return ((Integer) integrationResourcesService.getGlobalConfiguration().getEnv().getDatabase().get(
                "mongodb-quasar-timeout")).intValue();
    }
