...
public class OntologyReferencesValidationImp implements OntologyReferencesValidation {
    ...
    private void validateSingleReference(JsonNode reference, Ontology srcOntology, JsonNode srcSchema,
        ...
        if (sameType(srcSchema.at("/" + srcPath), dstSchema.at("/" + dstPath))) {
            if (!srcSchema.at("/" + srcPath).isArray() && !srcSchema.at("/" + srcPath).path("type").asText().equals(
                    "array")) {
                ...
                final String query = getQueryForValidation(srcSchema.at("/" + srcPath).path("type").asText(),
                                                           instance.findPath(srcField), dstPath, dstOnt, dstSchema,
                                                           dstOntology.getRtdbDatasource());
                if (!query.equals("")) {
                    ...
                    try {
                        if (mapper.readTree(value).findPath("value").asInt() == 0)
                            throw new OntologyDataJsonProblemException(
                                    "There are no instances of referenced ontology " + dstOntology.getIdentification() + " with value specified in field " + srcField);
                    } catch (final IOException e) {
                    ...
                    queries.stream().forEach(q -> {
                        ...
                        try {
                            if (mapper.readTree(value).path(0).path("value").asInt() == 0)
                                throw new OntologyDataJsonProblemException(
                                        "There are no instances of referenced ontology " + dstOntology.getIdentification() + " with values specified in field " + srcField);
                        } catch (final IOException e) {
    ...
    private boolean sameType(JsonNode srcProperty, JsonNode dstProperty) {
        final String srcType =
                srcProperty.isArray() ? srcProperty.get(0).path("type").asText() : srcProperty.path("type").asText();
        final String dstType =
                dstProperty.isArray() ? dstProperty.get(0).path("type").asText() : dstProperty.path("type").asText();
